 ...
 ****************************************************************************
 */
package weaver.interfaces.sap.reprocessinTFME.dao.st;
 ...
 *
 */
public class ReprocessInTFMEDaoImpl_ST implements ReprocessInTFMEDao_ST {
	String sql = "";
	String sql1 = "";
	...
	public List<ReprocessinTFMEDt2> getReprocessInTFMEAsZero(String zero, String id, Integer formId, String customer) {
		...
		RecordSetDataSource rsmis = new RecordSetDataSource("STMESDB");
		try {
			sql = " SELECT PRODUCTION_ORDER,PARENT_LOT_CODE,LOT_CODE,SUB_PRODUCTION_ORDER,MATERIAL,MAT_SHORT_DESC,QTY,WAFER_LOT,WAFER_LOT158,ASSY_LOT,BATCH,TRACE_CODE,WORK_WEEK,STORAGE_LOCATION,SALES_ORDER,SALES_ORDER_ITEM FROM  (SELECT  DISTINCT " 
					...
					+ "STS.LOT_ID AS LOT_CODE," + "STS.LOT_ID AS SUB_PRODUCTION_ORDER,"
					...
					+ "OSO.CUS_SO_ITEM AS SALES_ORDER_ITEM, "
					+ "ELT.CUST_RUN_ID AS WAFER_LOT, "
					+ "ELT.CUST_LOT_ID||'NFM' AS WAFER_LOT158, " + "ELT.ASSY_LOT_CODE AS ASSY_LOT,"
					+ "FUN_IERPRLT(STS.LOT_ID,'TRAN_BATCH_ID') AS BATCH,    " + "ELT.TRACE_CODE AS TRACE_CODE,"
					+ "ELT.WORK_WEEK AS WORK_WEEK,"
					...
					+ "(SELECT CUST_LOT_ID||'NFM' FROM MWIPELTSTS WHERE LOT_ID=MRG.FROM_TO_LOT_ID) AS WAFER_LOT158, "
					...
					+ "AND MLI.MIXED_LOT_ID<>MRG.FROM_TO_LOT_ID AND OBX.FACTORY='FGS' " + ") ) ORDER BY PARENT_LOT_CODE,LOT_CODE  "; // 查询没有装箱的拆分后的数据  
			rsmis.execute(sql);
			...
			while (rsmis.next()) {
				...
				sql = "INSERT INTO FORMTABLE_MAIN_" + formId
						...
						+ "','" + repData.getDescription() + "','" + repData.getWorkWeek() + "','" + repData.getSalesOrderItem() +  "')";				
				rs.execute(sql);
				rs.next();
	...
	public List<ReprocessinTFMEDt2> getReprocessInTFMEAsYzx(String yzx, String id, Integer formId, String customer) {
		...
		try {
			sql = " SELECT PRODUCTION_ORDER,LOT_CODE,SUB_PRODUCTION_ORDER,PARENT_LOT_CODE,MATERIAL,MAT_SHORT_DESC,WAFER_LOT,WAFER_LOT158,ASSY_LOT,BATCH,TRACE_CODE,WORK_WEEK,STORAGE_LOCATION,SALES_ORDER,SALES_ORDER_ITEM,SUM(MIXED_QTY) AS QTY,BOX_NO FROM  "
					...
					+ "(SELECT MAT_SHORT_DESC FROM MWIPMATDEF WHERE  MAT_ID=STS.MAT_ID AND FACTORY=STS.FACTORY ) AS MAT_SHORT_DESC,"
					+ "MIXED_QTY , " + "ELT.CUST_RUN_ID AS WAFER_LOT, " + "ELT.CUST_LOT_ID||'NFM' AS WAFER_LOT158,"
	...
	public List<ReprocessinTFMEDt3> merge(Integer mergeMode, String id, Integer formId) {
		...
		try {
			if (mergeMode == 0) {// 不合并
				...
				rs.execute(sql);
				sql = "SELECT QTY,DEST_BATCH,WAFER_LOT,BOX_NO,ASSY_LOT,WORK_WEEK,TRACE_CODE FROM FORMTABLE_MAIN_" + formId
						+ "_DT2 WHERE MAINID = '" + id + "'";
				rs.execute(sql);
				while (rs.next()) {
					repData.setMainid(Integer.parseInt(id));
					...
					sql = "INSERT INTO FORMTABLE_MAIN_" + formId
							+ "_DT3 (MAINID,QTY,FINAL_BATCH,ASSY_LOT,BOX_NO,WAFER_LOT,WORK_WEEK,TRACE_CODE) VALUES (" + repData.getMainid()
							+ "," + repData.getQty() + ",'" + repData.getFinalBatch() + "','" + repData.getAssyLot()
							+ "','" + repData.getBoxNo() + "','" + repData.getWaferLot() + "','" + repData.getWorkWeek() + "','" + repData.getTraceCode() + "')";
					rs.execute(sql);
			...
			if (mergeMode == 1) { // 按扩散批号合并
				...
				while (rs.next()) {
					repData.setMainid(Integer.parseInt(id));
					repData.setFinalBatch(rs.getString("DEST_BATCH"));
					sql1 = "SELECT ASSY_LOT,BOX_NO,WORK_WEEK,TRACE_CODE FROM FORMTABLE_MAIN_" + formId + "_DT2 WHERE MAINID = '" + id
							+ "' AND DEST_BATCH = '" + rs.getString("DEST_BATCH") + "' AND BATCH = DEST_BATCH";
					rs1.execute(sql1);
					rs1.next();
			...
			if (mergeMode == 2) { // 按组装批号合并
				...
				while (rs.next()) {
					...
					repData.setAssyLot(rs.getString("ASSY_LOT"));
					repData.setQty(rs.getInt("QTY"));
					sql1 = "SELECT WAFER_LOT,BOX_NO,WORK_WEEK,TRACE_CODE FROM FORMTABLE_MAIN_" + formId + "_DT2 WHERE MAINID = '" + id
							+ "' AND DEST_BATCH = '" + rs.getString("DEST_BATCH") + "' AND BATCH = DEST_BATCH";
					rs1.execute(sql1);
					rs1.next();
			...
			if (mergeMode == 4) { // 按年周号合并
				RecordSet rs = new RecordSet();
				RecordSet rs1 = new RecordSet();
				sql = "SELECT MIN(BATCH) AS DEST_BATCH,WORK_WEEK FROM FORMTABLE_MAIN_" + formId
						+ "_DT2 WHERE MAINID ='" + id + "' GROUP BY WORK_WEEK";
				Map<String, String> map = new HashMap<String, String>();
				rs.execute(sql);
				while (rs.next()) {
				...
				for (Entry<String, String> vo : map.entrySet()) {
					...
					basebean.writeLog("key:" + key);
					basebean.writeLog("value:" + value);
					sql = "SELECT WORK_WEEK FROM FORMTABLE_MAIN_" + formId + "_DT2 WHERE MAINID ='"
							+ id + "'";
					rs.execute(sql);
					while (rs.next()) {
				...
				while (rs.next()) {
					...
					repData.setQty(rs.getInt("QTY"));
					repData.setWorkWeek(rs.getString("WORK_WEEK"));
					sql1 = "SELECT ASSY_LOT,WAFER_LOT, BOX_NO,TRACE_CODE FROM FORMTABLE_MAIN_" + formId + "_DT2 WHERE MAINID = '" + id
							+ "' AND DEST_BATCH = '" + rs.getString("DEST_BATCH") + "' AND BATCH = DEST_BATCH";
					rs1.execute(sql1);
					rs1.next();
					repData.setAssyLot(rs1.getString("ASSY_LOT"));					
			...
			if (mergeMode == 5) { // 按整理编号合并
				...
				for (Entry<String, String> vo : map.entrySet()) {
					...
					basebean.writeLog("key:" + key);
					basebean.writeLog("value:" + value);
					sql = "SELECT TRACE_CODE FROM FORMTABLE_MAIN_" + formId + "_DT2 WHERE MAINID ='"
							+ id + "'";
					rs.execute(sql);
					while (rs.next()) {
				...
				while (rs.next()) {
					...
					repData.setQty(rs.getInt("QTY"));
					repData.setTraceCode(rs.getString("TRACE_CODE"));
					sql1 = "SELECT ASSY_LOT,WAFER_LOT, BOX_NO, WORK_WEEK FROM FORMTABLE_MAIN_" + formId + "_DT2 WHERE MAINID = '" + id
							+ "' AND DEST_BATCH = '" + rs.getString("DEST_BATCH") + "' AND BATCH = DEST_BATCH";
					rs1.execute(sql1);
					rs1.next();
					repData.setAssyLot(rs1.getString("ASSY_LOT"));					
