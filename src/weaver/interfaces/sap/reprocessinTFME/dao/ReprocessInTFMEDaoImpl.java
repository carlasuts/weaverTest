 ...
 * ****************************************************************************
 * System      : weaver
 * Module      : weaver.interfaces.dao.reprocessinTFME
 * File Name   : ReprocessInTFMEDaoImpl.java
 * Description : TODO
 * History
...
public class ReprocessInTFMEDaoImpl implements ReprocessInTFMEDao {
	...
	public List<ReprocessinTFMEDt2> getReprocessInTFMEAsZero(String zero, String id, Integer formId, String customer) {
		...
		RecordSetDataSource rsmis = new RecordSetDataSource("TESTDBST");
		try {
			sql = "SELECT  DISTINCT STS.ORDER_ID AS PRODUCTION_ORDER, STS.LOT_ID AS PARENT_LOT_CODE,"
					...
					+ "OSO.CUS_SO AS SALES_ORDER, " + "ELT.CUST_RUN_ID AS WAFER_LOT, "
					...
					+ "AND MLI.MIXED_LOT_ID<>MRG.FROM_TO_LOT_ID AND OBX.FACTORY='FGS' " + ")"; // 查询没有装箱的拆分后的数据
			rsmis.execute(sql);
			...
			while (rsmis.next()) {
				...
				sql = "INSERT INTO FORMTABLE_MAIN_" + formId
						+ "_DT2 (MAINID,QTY,LOT_CODE,PARENT_LOT_CODE,MATERIAL,BATCH,SUB_PRODUCTION_ORDER,PRODUCTION_ORDER,SALES_ORDER,ASSY_LOT,WAFER_LOT,STORAGE_LOCATION,BOX_NO,TRACE_CODE,DESCRIPTION) VALUES ("
	...
	public List<ReprocessinTFMEDt2> getReprocessInTFMEAsYzx(String yzx, String id, Integer formId, String customer) {
		...
		RecordSet rs = new RecordSet();
		try {
			sql = " SELECT PRODUCTION_ORDER,LOT_CODE,SUB_PRODUCTION_ORDER,PARENT_LOT_CODE,MATERIAL,MAT_SHORT_DESC,WAFER_LOT,WAFER_LOT158,ASSY_LOT,BATCH,TRACE_CODE,STORAGE_LOCATION,SALES_ORDER,SUM(MIXED_QTY) AS QTY,BOX_NO FROM  "
					...
					+ "GROUP BY PRODUCTION_ORDER,LOT_CODE,MATERIAL,MAT_SHORT_DESC,WAFER_LOT,WAFER_LOT158,ASSY_LOT,BATCH,TRACE_CODE,STORAGE_LOCATION,SALES_ORDER,BOX_NO,SUB_PRODUCTION_ORDER,PARENT_LOT_CODE";
			rsmis.execute(sql);
			ReprocessinTFMEDt2 repData = new ReprocessinTFMEDt2();
			while (rsmis.next()) {
				repData.setMainid(Integer.parseInt(id));
	...
	/**
	 * 根据选择的合并方式，合并明细表2中的数据，并插入到明细表3中
	 */
	@Override
	public List<ReprocessinTFMEDt3> merge(Integer mergeMode, String id, Integer formId) {
		...
		try {
			if (mergeMode == 0) {// 不合并
				...
				rs.execute(sql);
				sql = "SELECT QTY,DEST_BATCH,WAFER_LOT,BOX_NO,ASSY_LOT FROM FORMTABLE_MAIN_" + formId
				...
				while (rs.next()) {
					...
					sql = "INSERT INTO FORMTABLE_MAIN_" + formId
							+ "_DT3 (MAINID,QTY,FINAL_BATCH,ASSY_LOT,BOX_NO,WAFER_LOT) VALUES (" + repData.getMainid()
							+ "," + repData.getQty() + ",'" + repData.getFinalBatch() + "','" + repData.getAssyLot()
							+ "','" + repData.getBoxNo() + "','" + repData.getWaferLot() + "')";
					rs.execute(sql);
			...
			if (mergeMode == 1) { // 按扩散批号合并
				...
				while (rs.next()) {
					repData.setMainid(Integer.parseInt(id));
					repData.setFinalBatch(rs.getString("DEST_BATCH"));
					sql1 = "SELECT ASSY_LOT,BOX_NO FROM FORMTABLE_MAIN_" + formId + "_DT2 WHERE MAINID = '" + id
			...
			if (mergeMode == 2) { // 按组装批号合并
				...
				while (rs.next()) {
					...
					repData.setAssyLot(rs.getString("ASSY_LOT"));
					repData.setQty(rs.getInt("QTY"));
					sql1 = "SELECT WAFER_LOT,BOX_NO FROM FORMTABLE_MAIN_" + formId + "_DT2 WHERE MAINID = '" + id
							+ "' AND DEST_BATCH = '" + rs.getString("DEST_BATCH") + "' AND BATCH = DEST_BATCH";
					rs1.execute(sql1);
					rs1.next();
