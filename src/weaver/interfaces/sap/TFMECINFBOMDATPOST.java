...
public class TFMECINFBOMDATPOST implements Action {
	...
	public String execute(RequestInfo request) {
		...
		try {
			...
			DetailTable[] detailtable = request.getDetailTableInfo()
					.getDetailTable();
			CINFBOMDATPOST bom = new CINFBOMDATPOST();
			bom.setcinfbomdat(detailtable[2], rid, maintableinfo, creator);
			e.writeLog("cInfBomDatList接口数据处理");
			...
			baseBean.writeLog("------------------------------------------------------------------------");
			baseBean.writeLog("TFMECINFBOMDATPOST error:" + e.getMessage());
			baseBean.writeLog("------------------------------------------------------------------------");
