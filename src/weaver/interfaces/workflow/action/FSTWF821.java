...
//凭证审批流程
public class FSTWF821 implements Action {
	public String execute(RequestInfo request) {
		try {
			...
			baseBean.writeLog("FSTWF821 requestid:" + rid);
	        ...
	        baseBean.writeLog("FSTWF821 Belnr:" + Belnr);
	        baseBean.writeLog("FSTWF821 Gjahr:" + Gjahr);
	        baseBean.writeLog("FSTWF821 Bukrs:" + Bukrs);
	        baseBean.writeLog("FSTWF821 Zsentid:" + Zsentid);
	        baseBean.writeLog("FSTWF821 status:" + status);
	        baseBean.writeLog("FSTWF821 Zzaddapr:" + Zzaddapr);
			...
			if(status.equals("E")){
				...
				String err = result;
				baseBean.writeLog("FSTWF821 sapresult:" + result);
				if(result.length() > 0)
				...
				if(!result.toUpperCase().equals("S")){
			        ...
			        baseBean.writeLog("FSTWF821:SAP调用失败，请联系管理员");
				}
				
				sql = "update formtable_main_59 set fqbs = 1 where requestid = " + rid;
				baseBean.writeLog("FSTWF821 sql:" + sql);
				rs.executeSql(sql);
			}
		}
			...
			baseBean.writeLog("------------------------------------------------------------------------");
			baseBean.writeLog("FSTWF821 error:" + e.getMessage());
			baseBean.writeLog("------------------------------------------------------------------------");
