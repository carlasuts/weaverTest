...
public class FSTWF1186LCBH implements Action {
    public FSTWF1186LCBH() {
    }
...
//            Customer = rsmis.getString("key_1");
            ...
            if (fl < 10) {
                if (MaterialCode.length() > 11) {
                    lcbh = "A" + Customer + MaterialCode.substring(0, 12) + "-" + fl;
                } else {
                    lcbh = "A" + Customer + MaterialCode;
                ...
                String bhstr = fl + "";
                if (MaterialCode.length() > 11) {
                    lcbh = "A" + Customer + MaterialCode.substring(0, 11) + bhstr.substring(0, 1) + "-" + bhstr.substring(1, 2);
                } else {
    				lcbh = "A" + Customer + MaterialCode;
            ...
            String rcbh="A"+Customer+String.format("%04d", fl);
            ...
            while(flag != true){
            	if(rc != 0 ){
                	rcbh="A"+Customer+String.format("%04d", ++fl);
                	sql = "select count(*) as no from mwipflwdef where flow = '"+rcbh +"'";
                    mesrs.executeSql(sql);
                    mesrs.next();
                    rc = mesrs.getInt("no");
            ...
            while(flag1 != true){
            	if(pmc != 0 ){
            		...
            		if (mi < 10) {
                        if (MaterialCode.length() > 11) {
                            lcbh = "A" + Customer + MaterialCode.substring(0, 12) + "-" + mi;
                        } else {
                            lcbh = "A" + Customer + MaterialCode;
                            for (int i = MaterialCode.length(); i < 12; i++) {
                                lcbh += "9";
            ...
            baseBean.writeLog("------------------------------------------------------------------------");
            baseBean.writeLog("FSTWF1186LCBH error:" + var11.getMessage());
            baseBean.writeLog("------------------------------------------------------------------------");
