...
public class NTMEMES120501 implements Action {
	public String execute(RequestInfo request) {
        try {
	        ...
	        baseBean.writeLog("TFMEMES6610 requestid:" + rid);
...
//	            request.getRequestManager().setMessagecontent("客户未放行");
	        	String sql1 = "update formtable_main_9 set sfsj = 1 where requestid = " + rid;
				baseBean.writeLog("TFMEMES6610 sql:" + sql1);
				rs.executeSql(sql1);
	        }else{
	        	String sql1 = "update formtable_main_9 set sfsj = 0 where requestid = " + rid;
				baseBean.writeLog("TFMEMES6610 sql:" + sql1);
				rs.executeSql(sql1);
	        }
        }
	        ...
	        baseBean.writeLog("------------------------------------------------------------------------");
	        baseBean.writeLog("TFMEMES6610 error:" + e.getMessage());
	        baseBean.writeLog("------------------------------------------------------------------------");
