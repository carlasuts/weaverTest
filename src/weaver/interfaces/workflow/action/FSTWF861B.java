...
//固定资产流程
public class FSTWF861B implements Action {
	public String execute(RequestInfo request) {
		try {
			...
			baseBean.writeLog("FSTWF861B requestid:" + rid);
			sql = "select EWARNumber, sapid，SAPfh,zzspr from formtable_main_61 where requestid = " + rid;
			baseBean.writeLog("FSTWF861B sql:" + sql);
			rs.executeSql(sql);
			rs.next();
	        ...
	        baseBean.writeLog("FSTWF861B src:" + src);
	        ...
	        //status = "S";
	        baseBean.writeLog("FSTWF861B EWARNumber:" + EWARNumber);
	        baseBean.writeLog("FSTWF861B Zsentid:" + Zsentid);
	        baseBean.writeLog("FSTWF861B status:" + status);
	        baseBean.writeLog("FSTWF861B Zzaddapr:" + Zzaddapr);
	        baseBean.writeLog("FSTWF861B SAPfh:" + SAPfh);
			...
			if(SAPfh.equals("1")){
				...
				String err = result;
				baseBean.writeLog("FSTWF861B sapresult:" + result);
				if(result.length() > 0)
				...
				if(!result.toUpperCase().equals("S")){
			        ...
			        baseBean.writeLog("FSTWF861B:SAP调用失败，请联系管理员");
			        
			        sql = "update formtable_main_61 set SAPfh = 1 where requestid = " + rid;
			        baseBean.writeLog("FSTWF861B sql:" + sql);
					rs.executeSql(sql);
				}
				else{
			...
			baseBean.writeLog("------------------------------------------------------------------------");
			baseBean.writeLog("FSTWF861B error:" + e.getMessage());
			baseBean.writeLog("------------------------------------------------------------------------");
