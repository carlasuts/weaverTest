...
public class HFMEMES83SCH extends BaseCronJob  {
	private String logtablename = "HFYC_MIDDLE";
	private static final String url = "http://172.20.20.118:8090/MesWebService/req";
	...
	public void execute() {
		...
		try {
			...
			for(int i = 0; i < content.length(); i++){
				...
				String sql1 ="select id,flag from HFYC_MIDDLE where id="+ID;
				rs1.executeSql(sql1);
				if(rs1.next()){
...
//					writelog(loglist);
				...
				//khm xh pkid
				sql = "select b.id id,lastname,zrr from hf_gxnyc a inner join hrmresource b on a.workcode = b.workcode where 1 = 1";
				if(!CUST_ID.equals("")){
					sql += " and khm = '" + CUST_ID + "'";
				}
				if(!MAT_DESC.equals("")){
				...
				//khm xh tester pkid
				sql = "select b.id id,lastname,zrr from hf_llyc a inner join hrmresource b on a.workcode = b.workcode where 1 = 1";
				if(!CUST_ID.equals("")){
					sql += " and khm = '" + CUST_ID + "'";
				}
				if(!MAT_DESC.equals("")){
				...
				requestInfo.setCreatorid(USER_ID);
				requestInfo.setWorkflowid("1265");
				requestInfo.setDescription("合肥工序内异常处理流程-" + date + "(LOT:"+LOT_ID+")");
				
				requestInfo.setMainTableInfo(mainTableInfo);
				RequestService service = new RequestService();//创建请求服务实例
				
				String newRequestid = service.createRequest(requestInfo); 
				bs.writeLog("HFYC_MIDDLE_GXN:" + newRequestid);
				
				SimpleDateFormat sdf1 = new SimpleDateFormat("yyyyMMddHHmmss"); 
				String udpatetime = sdf1.format(d);
				sql = "update HFYC_MIDDLE set FLAG = '100',UPDATE_TIME = '" + udpatetime + "' where id = " + ID;
				bs.writeLog("HFYC_MIDDLE_GXN update sql:" + sql);
				rs1.executeSql(sql);	
			}
			else{
				bs.writeLog("良率异常");
				...
				requestInfo.setCreatorid(USER_ID);
				requestInfo.setWorkflowid("1266");
				requestInfo.setDescription("合肥良率异常处理流程-" + date + "(LOT:"+LOT_ID+")");
