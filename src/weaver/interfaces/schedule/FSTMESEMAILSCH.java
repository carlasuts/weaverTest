...
public class FSTMESEMAILSCH extends BaseCronJob  {
	public void execute() {
		...
		try {
			...
			sql = "select * from formtable_main_6 where requestid "
					+ "in(select requestid from workflow_requestbase "
					+ "where createdate < to_char((sysdate - interval '5' day),'yyyyMMdd')) "
					+ "and edpno in(select lot_id from llyc_middle "
					+ "where flag =100 and CREATE_DATE< to_char((sysdate - interval '5' day),'yyyyMMdd')) "
