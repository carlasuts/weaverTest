...
public class STMEMES83SCH extends BaseCronJob {
	private String logtablename = "STYC_MIDDLE";
	...
	public void execute() {
		...
		try {
			...
			for (int i = 0; i < content.length(); i++) {
				...
				String sql1 = "select id,flag from STYC_MIDDLE where id=" + ID;
				rs1.executeSql(sql1);
				if (rs1.next()) {

				} else {
					String sql = "";
					// List<Map<String, String>> loglist = new
					...
					// JSONArray content =
					// rjson.getJSONArray("oaTestYieldList");//根据文档填写
					bs.writeLog("-----------------------------");
					if (statusValue.equals("0")) {
						...
						if ((RESVFIELD5.equals("VD_LOW_YIELD")
								&& (!"BIN97".equals(TOPBIN1_PROMPT) && !"BIN99".equals(TOPBIN1_PROMPT)
										&& !"GXN".equals(TOPBIN1_PROMPT) && !"BROKEN".equals(TOPBIN1_PROMPT)
										&& !"LOST".equals(TOPBIN1_PROMPT) && !"VI DEFECT".equals(TOPBIN1_PROMPT)))
								|| (RESVFIELD5.equals("VD_LOW_YIELD") && (!"".equals(TOPBIN2_PROMPT)
										&& !"BIN97".equals(TOPBIN2_PROMPT) && !"GXN".equals(TOPBIN2_PROMPT)
										&& !"BIN99".equals(TOPBIN2_PROMPT) && !" ".equals(TOPBIN2_PROMPT)
										&& !"BROKEN".equals(TOPBIN2_PROMPT) && !"VI DEFECT".equals(TOPBIN2_PROMPT)
										&& !"LOST".equals(TOPBIN2_PROMPT)))
								|| (RESVFIELD5.equals("VD_LOW_YIELD")
										&& (!"".equals(TOPBIN3_PROMPT) && !" ".equals(TOPBIN3_PROMPT)))) {
							...
							// khm xh pkid
							sql = "select b.id id,lastname,zrr from st_gxnyc a inner join hrmresource b on a.workcode = b.workcode where 1 = 1";
							if (!CUST_ID.equals("")) {
								sql += " and khm = '" + CUST_ID + "'";
							}
							if (!MAT_DESC.equals("")) {
							...
							// khm xh tester pkid
							sql = "select b.id id,lastname,zrr from st_llyc a inner join hrmresource b on a.workcode = b.workcode where 1 = 1";
							if (!CUST_ID.equals("")) {
								sql += " and khm = '" + CUST_ID + "'";
							}
							if (!MAT_DESC.equals("")) {
							...
							requestInfo.setWorkflowid("1206");
							requestInfo.setDescription("苏通工序内+良率异常处理流程-" + date + "(LOT:" + LOT_ID + ")");

							requestInfo.setMainTableInfo(mainTableInfo);
							RequestService service = new RequestService();// 创建请求服务实例

							String newRequestid = service.createRequest(requestInfo);
							bs.writeLog("STYC_MIDDLE:" + newRequestid);

							SimpleDateFormat sdf1 = new SimpleDateFormat("yyyyMMddHHmmss");
							String udpatetime = sdf1.format(d);
							sql = "update STYC_MIDDLE set FLAG = '100',UPDATE_TIME = '" + udpatetime + "' where id = "
									+ ID;
							bs.writeLog("STYC_MIDDLE update sql:" + sql);
							rs1.executeSql(sql);
						} else if (TOTAL_YIELD.equals(" ") || TOTAL_YIELD.equals("")
								|| (RESVFIELD5.equals("VD_LOW_YIELD"))) {
							// null == TOTAL_YIELD ||
							// "".equals(TOTAL_YIELD.trim())
							bs.writeLog("工序内异常");
							// 触发工序内异常流程
							...
							requestInfo.setCreatorid(USER_ID);
							requestInfo.setWorkflowid("1205");
							requestInfo.setDescription("苏通工序内异常处理流程-" + date + "(LOT:" + LOT_ID + ")");

							requestInfo.setMainTableInfo(mainTableInfo);
							RequestService service = new RequestService();// 创建请求服务实例

							String newRequestid = service.createRequest(requestInfo);
							bs.writeLog("STYC_MIDDLE_GXN:" + newRequestid);

							SimpleDateFormat sdf1 = new SimpleDateFormat("yyyyMMddHHmmss");
							String udpatetime = sdf1.format(d);
							sql = "update STYC_MIDDLE set FLAG = '100',UPDATE_TIME = '" + udpatetime + "' where id = "
									+ ID;
							bs.writeLog("STYC_MIDDLE_GXN update sql:" + sql);
							rs1.executeSql(sql);
						} else {
							bs.writeLog("良率异常");
							// 触发良率异常流程
							...
							requestInfo.setCreatorid(USER_ID);
							requestInfo.setWorkflowid("1204");
							requestInfo.setDescription("苏通良率异常处理流程-" + date + "(LOT:" + LOT_ID + ")");
	...
	public static String readInterfacePost(String detailUrl, String str) {
		...
		try {
			// post.setRequestHeader("User-Agent", "Mozilla/5.0 (Windows NT 6.1;
