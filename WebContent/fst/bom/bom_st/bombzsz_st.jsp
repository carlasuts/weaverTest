...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",bombzszupdate_st.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="bombzsz_st.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='bombzsz_st.xls';"></td>
		</TR>
...
//修改
function doEdit(id){
	document.frmmain.action = "bombzszupdate_st.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("bombzszexcel_st.jsp");
	if(rvalue > 0){
		window.location.href = "bombzsz_st.jsp";
	}
}
function doDownload(){
	window.location.href='bombzsz_st.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="bombzszoperation_st.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
