...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="testerBrowser.jsp" method=post>
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'  attributes="{'groupSHBtnDisplay':'none'}" >
		<wea:item>测试机</wea:item>
		<wea:item><input name="tester" value='<%=tester%>' class="InputStyle"></wea:item>
	</wea:group>
</wea:layout>
</div>
...
String tableString = "";
int perpage=10;
String backfields = " distinct tester ";
String fromSql  = " (select b.name as tester, QTTN_PKG_FK from MD_SD_QTTN_PKG_TESTER a right join MD_TESTER b on A.TESTER_FK = B.ID) ";
	tableString =   " <table instanceid=\"db_list3\" tabletype=\"none\" pagesize=\""+perpage+"\"  >"+ //指定分页条数和初始化id以及是否有复选框
			...
			" sqlprimarykey=\"tester\" sqlsortway=\"asc\" sqlisdistinct=\"false\" />"+
                "       <head>"+
                "           <col width=\"0%\" hide=\"true\"  text=\""+"tester"+"\" column=\"tester\"    />"+
	        	"           <col width=\"100%\"  text=\"测试机\" column=\"tester\" orderkey=\"tester\"   />"+
                "       </head>"+
                " </table>";
%>
