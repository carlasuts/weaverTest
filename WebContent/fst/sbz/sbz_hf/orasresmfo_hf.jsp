...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",orasresmfoupdate_hf.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="orasresmfo_hf.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='orasresmfo_hf.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " orasresmfo_hf b ";
...
//修改
function doEdit(id){
	document.frmmain.action = "orasresmfoupdate_hf.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("orasresmfoexcel_hf.jsp");
	if(rvalue > 0){
		window.location.href = "orasresmfo_hf.jsp";
	}
}
function doDownload(){
	window.location.href='orasresmfo_hf.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="orasresmfooperation_hf.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
