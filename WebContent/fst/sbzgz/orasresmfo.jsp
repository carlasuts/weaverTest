...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",orasresmfoupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="orasresmfo.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='orasresmfo.xls';"></td>
		</TR>
...
//修改
function doEdit(id){
	document.frmmain.action = "orasresmfoupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("orasresmfoexcel.jsp");
	if(rvalue > 0){
		window.location.href = "orasresmfo.jsp";
	}
}
function doDownload(){
	window.location.href='orasresmfo.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="orasresmfooperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
