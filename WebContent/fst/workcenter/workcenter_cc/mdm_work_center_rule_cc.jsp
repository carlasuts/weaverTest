...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",mdm_work_center_rule_update_cc.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="mdm_work_center_rule_cc.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='mdm_work_center_rule_cc.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " mdm_work_center_rule_cc R ";
...
//修改
function doEdit(id){
	document.frmmain.action = "mdm_work_center_rule_update_cc.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("mdm_work_center_rule_excel_cc.jsp");
	if(rvalue > 0){
		window.location.href = "mdm_work_center_rule_cc.jsp";
	}
}
function doDownload(){
	window.location.href='mdm_work_center_rule_cc.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="mdm_work_center_rule_operation_cc.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
