...
<%@include file="/systeminfo/init_wev8.jsp" %>
   <%
    ...
    if(type.equals("del")){
	    sql = "delete mdm_work_center_rule where id = '"+id+"'";
	    rs.executeSql(sql);
    }
    else if(type.equals("modi")){
    	sql = "update  mdm_work_center_rule set CUST_FK = '" + CUST_FK + "',PKG_OUTLINE_FK = '" + PKG_OUTLINE_FK + "',OPER_FK = '" + OPER_FK + "' ,VEHICLE_FK = '" + VEHICLE_FK + "',WORK_CENTER_FK = '" + WORK_CENTER_FK + "',DATE_EFFECT = TO_DATE('" + DATE_EFFECT + "','yyyy-MM-dd HH24:mi:ss') where id = '"+id+"'";
		rs.executeSql(sql);
    }
    else if(type.equals("add")){
    	sql = "Insert into MDM_WORK_CENTER_RULE (ID, CUST_FK, PKG_OUTLINE_FK, OPER_FK, VEHICLE_FK, WORK_CENTER_FK, DATE_CREATE, DATE_UPDATE, DATE_EFFECT, DATE_EXPIRE) Values ( " +
				"SYS_GUID()||'',"+ 
    				"'" + CUST_FK + "'," +
    				"'" + PKG_OUTLINE_FK + "'," +
    				"'" + OPER_FK + "'," +
    ...
    response.sendRedirect("mdm_work_center_rule.jsp");
   %>
