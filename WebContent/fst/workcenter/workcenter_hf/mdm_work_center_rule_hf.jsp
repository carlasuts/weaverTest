...
if(!CUST_FK.equals("")){
	sqlwhere += " and CUST_FK like '%" + CUST_FK + "%'";
}
...
if(!VEHICLE_FK.equals("")){
	sqlwhere += " and VEHICLE_FK like '%" + VEHICLE_FK + "%'";
}
...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",mdm_work_center_rule_update_hf.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="mdm_work_center_rule_hf.jsp" method=post>
		...
		<TR>
			...
			<input type=text id='CUST_FK' name='CUST_FK' value='<%=CUST_FK%>'></td>
			...
			<td NOWRAP width='5%'>车载品</td>
			<td class=FIELD width='10%'>
			<input type=text id='VEHICLE_FK' name='VEHICLE_FK' value='<%=VEHICLE_FK%>'></td>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='mdm_work_center_rule_hf.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " mdm_work_center_rule_hf R ";
...
//修改
function doEdit(id){
	document.frmmain.action = "mdm_work_center_rule_update_hf.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("mdm_work_center_rule_excel_hf.jsp");
	if(rvalue > 0){
		window.location.href = "mdm_work_center_rule_hf.jsp";
	}
}
function doDownload(){
	window.location.href='mdm_work_center_rule_hf.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="mdm_work_center_rule_operation_hf.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
