...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="gxn_llyc.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='gxn_llyc.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " TB_GXN_LLYC a left join hrmresource b on a.workcode = b.workcode ";
...
function doImportExcel(){
	var rvalue = window.showModalDialog("gxn_llycexcel.jsp");
	if(rvalue > 0){
		window.location.href = "gxn_llyc.jsp";
	}
}
function doDownload(){
	window.location.href='gxn_llyc.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="gxn_llycoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
