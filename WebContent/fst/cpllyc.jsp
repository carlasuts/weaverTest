...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="cpllyc.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='cpllyc.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " tb_cpllyc a left join hrmresource b on a.workcode = b.workcode ";
...
function doImportExcel(){
	var rvalue = window.showModalDialog("cpllycexcel.jsp");
	if(rvalue > 0){
		window.location.href = "cpllyc.jsp";
	}
}
function doDownload(){
	window.location.href='cpllyc.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="cpllycoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
