...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",MDM_OPER_MTRL_REFupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="MDM_OPER_MTRL_REF.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='MDM_OPER_MTRL_REF.xls';"></td>
		</TR>
...
function doImportExcel(){
	var rvalue = window.showModalDialog("MDM_OPER_MTRL_REFexcel.jsp");
	if(rvalue > 0){
		window.location.href = "MDM_OPER_MTRL_REF.jsp";
	}
}
function doDownload(){
	window.location.href='bombzsz.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="MDM_OPER_MTRL_REFoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
