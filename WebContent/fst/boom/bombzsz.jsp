...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",bombzszupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="bombzsz.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='bombzsz.xls';"></td>
		</TR>
...
//修改
function doEdit(id){
	document.frmmain.action = "bombzszupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("bombzszexcel.jsp");
	if(rvalue > 0){
		window.location.href = "bombzsz.jsp";
	}
}
function doDownload(){
...
function doDel(id){
	...
	if(isdel){
		document.frmmain.action="bombzszoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
