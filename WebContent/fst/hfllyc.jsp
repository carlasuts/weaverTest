...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="hfllyc.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='hfllyc.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " hf_llyc a left join hrmresource b on a.workcode = b.workcode ";
...
function doImportExcel(){
	var rvalue = window.showModalDialog("hfllycexcel.jsp");
	if(rvalue > 0){
		window.location.href = "hfllyc.jsp";
	}
}
function doDownload(){
	window.location.href='hfllyc.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="hfllycoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
