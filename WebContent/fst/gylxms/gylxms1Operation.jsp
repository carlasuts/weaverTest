...
<%
	...
	String item_no =  Util.fromScreen(request.getParameter("item_no"),user.getLanguage());
   	...
   	String oper_desc =  Util.fromScreen(request.getParameter("oper_desc"),user.getLanguage());
	...
	String sql = "";
	if(id!= null){
		sql = "select master_fk from MD_MDM_STD_ROUTER_item where id = "  + id;
		bb.writeLog(sql);
		rs.executeSql(sql);
		rs.next();
	...
	if(type.equals("del")){
		sql = "delete MD_MDM_STD_ROUTER_item where id  = " + id;
		rs.executeSql(sql);
	}
	...
	response.sendRedirect("gylxms1list.jsp?id="+master_fk);
%>