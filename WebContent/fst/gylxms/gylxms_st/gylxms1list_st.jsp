...
<%
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",gylxmsupdate_st.jsp?id="+id1+",_self} " ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="gylxms1list_st.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='gylxms_st.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " MD_MDM_STD_ROUTER_item_st ";
...
function doImportExcel(){
	var rvalue = window.showModalDialog("gylxmsexcel_st.jsp?");
	if(rvalue > 0){
		window.location.href = "gylxms1list_st.jsp";
	}
}

function doDownload(){
	window.location.href='gylxms_st.xls';
}

function doDel(a){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="gylxms1Operation_st.jsp?type=del&a='"+a+"'";
		document.frmmain.submit();	
	}
}
</script>
