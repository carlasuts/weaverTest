...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",gylxms.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
%>
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="gylxmslist.jsp" method=post>
...
function doEdit(id){
	document.frmmain.action = "gylxms.jsp?id="+id;
	document.frmmain.submit();
}
//明细
function doView(id){
	document.frmmain.action = "gylxms1list.jsp?id="+id;
	document.frmmain.submit();
}
//删除
function doDel(id){
	if(confirm("删除后二级也会被删除！真的要删除吗？")){
		document.frmmain.action = "gylxmsOperation.jsp?type=del&id="+id;
		document.frmmain.submit();
	}
}
</script>
