...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",gylxms_hf.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
%>
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="gylxmslist_hf.jsp" method=post>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " MD_MDM_STD_ROUTER_hf b ";
...
function doEdit(id){
	document.frmmain.action = "gylxms_hf.jsp?id="+id;
	document.frmmain.submit();
}
//明细
function doView(id){
	document.frmmain.action = "gylxms1list_hf.jsp?id="+id;
	document.frmmain.submit();
}
//删除
function doDel(id){
	if(confirm("删除后二级也会被删除！真的要删除吗？")){
		document.frmmain.action = "gylxmsOperation_hf.jsp?type=del&id="+id;
		document.frmmain.submit();
	}
}
</script>
