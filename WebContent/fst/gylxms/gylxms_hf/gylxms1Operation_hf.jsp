...
<%
	...
	String sql = "";
	if(id!= null){
		sql = "select master_fk from MD_MDM_STD_ROUTER_item_hf where id = "  + id;
		bb.writeLog(sql);
		rs.executeSql(sql);
		rs.next();
	...
	if(type.equals("del")){
		sql = "delete MD_MDM_STD_ROUTER_item_hf where id  = " + id;
		rs.executeSql(sql);
	}
	 else if(type.equals("add")){
	    	sql = "insert into MD_MDM_STD_ROUTER_item_hf(id,master_fk,item_no,oper_fk,factory) values (SYS_GUID()||'',"+
					"'" + master_fk + "'," +
					"'" + item_no + "'," +
					"'" + oper_fk + "'," +
					"'" + factory + "'" +
	...
	response.sendRedirect("gylxms1list_hf.jsp?id="+master_fk);
%>