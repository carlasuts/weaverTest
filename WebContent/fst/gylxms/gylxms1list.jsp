...
<%
	...
	String oper_desc = Util.null2String(request.getParameter("oper_desc"));//
	...
	//
	if(!oper_desc.equals("")){
		sqlwhere += " and oper_desc like '%" + oper_desc + "%'";
	}
...
<%
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",gylxmsupdate.jsp?id="+id1+",_self} " ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="gylxms1list.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='gylxms.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		        	<%
		            String backfields = "id, item_no,oper_fk,factory,oper_desc";
		            String fromSql  = " MD_MDM_STD_ROUTER_item ";
		            String sqlWhere = sqlwhere;
		            String orderby = " oper_fk " ;
		            String tableString = "";
		            tableString =" <table instanceid=\"contract\" tabletype=\"none\" pagesize=\""+perpage+"\" >"+
								 ...
								 "			<col width=\"15%\" text=\"描述\" column=\"oper_desc\" orderkey=\"oper_desc\"  />"+
		                         "		</head>"+
								 "		<operates width=\"20%\">";
...
function doImportExcel(){
	var rvalue = window.showModalDialog("gylxmsexcel.jsp?");
	if(rvalue > 0){
		window.location.href = "gylxms1list.jsp";
	}
}

function doDownload(){
	window.location.href='gylxms.xls';
}

function doDel(a){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="gylxms1Operation.jsp?type=del&a='"+a+"'";
		document.frmmain.submit();	
	}
}
</script>
