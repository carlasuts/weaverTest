...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="uph_cc.jsp" method=post>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " MDM_UPH_RULE_cc ";
...
//修改
function doEdit(id){
	document.frmmain.action = "uphupdate_cc.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("uphexcel_cc.jsp");
	if(rvalue > 0){
		window.location.href = "uph_cc.jsp";
	}
}
function doDownload(){
	window.location.href='uph_cc.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="uphoperation_cc.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
