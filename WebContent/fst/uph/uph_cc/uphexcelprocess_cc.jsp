...
<%
	...
	while( true ) {
		...
		//根据封装外形工序确定唯一
		sql = "select * from MDM_UPH_RULE where PKG_OUTLINE_FK = '" + c2 + "' and OPER_FK = '" + c3 + "' ";
		rs1.executeSql(sql);
		if(!rs1.next()){
			sql = "insert into MDM_UPH_RULE (id,PKG_OUTLINE_FK,OPER_FK,SINGLE_ATTACH_TIME,ATTACH_CHANGE_WAFER_TIME,SINGLE_WIRE_BOND_TIME,SINGLE_NO_WIRE_TIME,SINGLE_GRIND_TIME,SCRIBE_METERS,SCRIBE_CHANGE_WAFER_TIME,UPH_FORMULA) values (SYS_GUID()||'',"+
			"'" + c2 + "'," +
			"'" + c3 + "'," +
			"'" + c4 + "'," +
			"'" + c5 + "'," +
			...
			")";
			rs.executeSql(sql); 
				bb.writeLog("MDM_UPH_RULE insertsql:" + sql);
		}
		else{
		sql = "update MDM_UPH_RULE set SINGLE_ATTACH_TIME = '"+ c4 +"',ATTACH_CHANGE_WAFER_TIME = '"+ c5 +"'  ,SINGLE_WIRE_BOND_TIME = '"+ c6 +"',SINGLE_NO_WIRE_TIME = '"+ c7 +"',SINGLE_GRIND_TIME = '"+ c8 +"',SCRIBE_METERS= '"+ c9 +"',SCRIBE_CHANGE_WAFER_TIME = '"+ c10 +"',UPH_FORMULA = '"+ c11 +"' where PKG_OUTLINE_FK = '" + c2 + "' and OPER_FK = '" + c3 + "' ";
	    rs2.executeSql(sql);
	    bb.writeLog("MDM_UPH_RULE updatesql:" + sql);
		}
}
