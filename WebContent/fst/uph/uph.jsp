...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",uphupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="uph.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='uph.xls';"></td>
		</TR>
...
//修改
function doEdit(id){
	document.frmmain.action = "uphupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("uphexcel.jsp");
	if(rvalue > 0){
		window.location.href = "uph.jsp";
	}
}
function doDownload(){
...
function doDel(id){
	...
	if(isdel){
		document.frmmain.action="uphoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
