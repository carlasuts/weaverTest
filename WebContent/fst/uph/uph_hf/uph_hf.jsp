...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",uphupdate_hf.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="uph_hf.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='uph_hf.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " MDM_UPH_RULE_hf ";
...
//修改
function doEdit(id){
	document.frmmain.action = "uphupdate_hf.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("uphexcel_hf.jsp");
	if(rvalue > 0){
		window.location.href = "uph_hf.jsp";
	}
}
function doDownload(){
	window.location.href='uph.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="uphoperation_hf.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
