	...
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'  attributes="{'groupSHBtnDisplay':'none'}" >
		...
		<wea:item>
			<brow:browser name="oper_fk" viewType="0" hasBrowser="true" hasAdd="false"
						  browserUrl='/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.jsgx'
						  isMustInput="1"
						  isSingle="false"
						  hasInput="true"
...
String fromSql  = " (select to_char(c.id) as id,  g.ITEMNO,g.operadd,c.mat_id ,c.mat_desc, c.mat_type   ,c.storage_location \n" +
		"FROM obomsetmat c ,   \n" +
		"(select f.ITEMNO,f.operadd ,mp.oper_grp_4 from formtable_main_78 e,formtable_main_78_dt2 f ,mwipoprdef mp  \n" +
		...
		"union  \n" +
		"select seq, g.ITEMNO,g.operadd,e.mat_id,e.full_name,e.MAT_TYPE_FK ,e.plant_location \n" +
		"from BOMINFO e ,MDM_OPER_MTRL_REF f, \n" +
		"(select f.ITEMNO,f.operadd from formtable_main_78 e,formtable_main_78_dt2 f  \n" +
		"  where e.requestid =  " + requestid + " \n" +
		"  and  f.mainid = e.id  ) g  \n" +
		"where e.MTRL_GROUP_FK = f.mtrl_group_fk \n" +
		"and g.operadd in (select distinct wl.oper_fk from hrmrolemembers gh,hrmroles js,MDM_OPER_MTRL_REF wl \n" +
		"  where gh.resourceid = " + resourceid + " \n" +
		"  and gh.roleid = js.id \n" +
		"  and js.rolesmark = wl.oper_role_fk )) ";
e1.writeLog(fromSql);
	tableString =   " <table instanceid=\"db_list3\" tabletype=\"checkbox\" pagesize=\"7\" >"+ //指定分页条数和初始化id以及是否有复选框
			...
			" sqlorderby=\""+orderby+"\"   sqlprimarykey=\"id\" sqlsortway=\"asc\" sqlisdistinct=\"false\" />"+
	        	...
	        	"           <col width=\"100%\"  text=\"工序项目号\" column=\"ITEMNO\" orderkey=\"ITEMNO\"   />"+
		        "           <col width=\"100%\"  text=\"工序\" column=\"operadd\" orderkey=\"operadd\"   />"+
			    "           <col width=\"100%\"  text=\"料号\" column=\"mat_id\" orderkey=\"mat_id\"   />"+
			    "           <col width=\"100%\"  text=\"料号描述\" column=\"mat_desc\" orderkey=\"mat_desc\"   />"+
				"           <col width=\"100%\"  text=\"物料类型\" column=\"mat_type\" orderkey=\"mat_type\"   />"+
				"           <col width=\"100%\"  text=\"仓储地点\" column=\"storage_location\" orderkey=\"storage_location\"   />"+
...
jQuery(document).ready(function(){
	...
	if(jQuery("#isfirst").val()==''){
		jQuery("#cust_id").val(parentWin.document.getElementById("field11394").value);
		jQuery("#pkld").val(parentWin.document.getElementById("field11521").value);
