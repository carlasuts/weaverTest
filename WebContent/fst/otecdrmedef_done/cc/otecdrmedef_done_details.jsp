...
<%
	...
	if(!p_id.equals("")){
		rs.executeSql("select custid,TZLXNAME,tzbh,tzbb,pkg,pkld,CADGSTZ,YSZLSC,REQUESTID from otecdrmedef_cc  where tzbh = '"+ p_id +"' and (tzbb, tzbh) in (select max(To_Number(tzbb)),tzbh from  otecdrmedef_cc group by tzbh)  ");
		if(rs.next()){
...
<BODY>
	...
	<%
		...
		//返回
		RCMenu += "{" + SystemEnv.getHtmlLabelName(1290, user.getLanguage()) + ",javascript:location.href='otecdrmedef_done.jsp',_self} ";
		RCMenuHeight += RCMenuHeightStep;
	%>
	...
	<table width=100% height=100% border="0" cellspacing="0"
		...
		<tr>
			...
			<td valign="top">
				...
				<input type=hidden id = 'cadid' name='IMAGEFILEID_cad' value='<%=IMAGEFILEID_cad%>'>
				<input type=hidden id = 'pdfid' name='IMAGEFILEID_ys' value='<%=IMAGEFILEID_ys%>'>	
...
<script type="text/javascript">
	jQuery(function () {
		if($('#cadid').val() == ''){
			$("td:contains('CAD附件预览/下载：')").next().empty();
			$("td:contains('CAD附件预览/下载：')").next().append('<td align="left">无CAD附件</td>');
		}
		if($('#pdfid').val() == ''){
			$("td:contains('原始资料附件预览/下载：')").next().empty();
			$("td:contains('原始资料附件预览/下载：')").next().append('<td align="left">无PDF附件</td>');
		}
	}); 
</script>
