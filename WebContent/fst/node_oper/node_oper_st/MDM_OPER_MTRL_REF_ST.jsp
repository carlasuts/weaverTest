...
String sqlwhere = " where 1 = 1 AND SITE_OPTION = '4000' ";
...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",MDM_OPER_MTRL_REFupdate_ST.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="MDM_OPER_MTRL_REF_ST.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='MDM_OPER_MTRL_REF_ST.xls';"></td>
		</TR>
...
//修改
function doEdit(id){
	document.frmmain.action = "MDM_OPER_MTRL_REFupdate_ST.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("MDM_OPER_MTRL_REFexcel_ST.jsp");
	if(rvalue > 0){
		window.location.href = "MDM_OPER_MTRL_REF_ST.jsp";
	}
}
function doDownload(){
	window.location.href='MDM_OPER_MTRL_REF_ST.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="MDM_OPER_MTRL_REFoperation_ST.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
