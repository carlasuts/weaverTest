...
<%
	...
	while( true ) {
        ...
        sql="select id from md_sd_wire where name ='"+c2+"'";
        rs.executeSql(sql); 
        while(rs.next()){
        	String WIRE_FK ="";
        	WIRE_FK = rs.getString("id");
        }
        
        sql="select id from MD_SD_WIRE_DIAM where full_name ='"+c3+"'";
        rs.executeSql(sql); 
        while(rs.next()){
        	String WIRE_DIAM_FK ="";
        	WIRE_DIAM_FK=rs.getString("id");
        }
        
        	sql = "insert into MD_SD_WIRE_UNIT_PIRCE ( id,WIRE_FK,WIRE_DIAM_FK,PIRCE,PIRCE_STEP,PIRCE_DIFF) values (SYS_GUID()||'',"+
    				"'" + WIRE_FK + "'," +
    				"'" + WIRE_DIAM_FK + "'," +
    				"'" + c4 + "'," +
    				"'" + c5 + "'," +
    				...
    				")";
    	    rs.executeSql(sql); 
            bb.writeLog("插入 MD_SD_WIRE_UNIT_PIRCE:" + sql);
        //}
        //else{
