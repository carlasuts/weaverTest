...
<%
	...
	String QTTN_PKG_FK = "",name = "",MAINTENANCE_COST= "",LABOUR= "",POWER= "",DEPR= "",OTHER= "",FRAME_FK="";

	RecordSet rs = new RecordSet();
	if(!id.equals("")){
		rs.executeSql("select a.QTTN_PKG_FK,b.name,MAINTENANCE_COST,LABOUR  ,POWER  ,DEPR,OTHER,FRAME_FK from MD_SD_FRM_PKG a left join MD_SD_FRAME b on A.FRAME_FK = b.id where a.id ='"+id+"'");
		if(rs.next()){
			...
			FRAME_FK = rs.getString("FRAME_FK");
		}
	}
	String type = "";
	if (id.equals("")) {
							...
							<TR>
								...
								<p>
									<brow:browser name="name" viewType="0" hasBrowser="true" hasAdd="false"
										  browserUrl='/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.framework_tech'
						 				  isMustInput="1"
						  				  isSingle="false"
						 				  hasInput="true"
										  completeUrl=""  width="200px"
										  browserValue='<%=FRAME_FK %>'
