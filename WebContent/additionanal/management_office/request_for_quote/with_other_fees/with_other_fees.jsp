...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",with_other_feesupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="with_other_fees.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='with_other_fees.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		        	<%
		            String backfields = " a.id,a.QTTN_PKG_FK,b.name,MAINTENANCE_COST,LABOUR ,POWER ,DEPR ,OTHER  ";
		            String fromSql  = " MD_SD_FRM_PKG a left join MD_SD_FRAME b on A.FRAME_FK = b.id";
		            ...
		            tableString =" <table instanceid=\"contract\" tabletype=\"none\" pagesize=\""+perpage+"\" >"+
								...
								"			<col width=\"10%\" text=\"外形\" column=\"QTTN_PKG_FK\" orderkey=\"QTTN_PKG_FK\"  />"+
								 "			<col width=\"10%\" text=\"Frame\" column=\"name\" orderkey=\"name\"  />"+
...
//修改
function doEdit(id){
	document.frmmain.action = "with_other_feesupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("with_other_feesexcel.jsp");
	if(rvalue > 0){
		window.location.href = "with_other_fees.jsp";
	}
}
function doDownload(){
	window.location.href='with_other_fees.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="with_other_feesoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
