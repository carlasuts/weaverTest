...
<%
	...
	while( true ) {
        ...
        //rs.executeSql(sql);
        //if(!rs.next()){
        sql ="select id from MD_APPROVEL_LEVEL where FULL_NAME = '"+c3+"'";
        rs.executeSql(sql);
        while(rs.next()){
        	String APPROVEL_LEVEL_FK =rs.getString("id");
        }
        
         sql ="select id from MD_SD_QTTN_REASON where name = '"+c4+"'";
        rs.executeSql(sql);
        while(rs.next()){
        	String FPA_REASON =rs.getString("id");
        }
        	sql = "insert into MD_SD_GPM_RANG_AR (id,name,APPROVEL_LEVEL_FK,FPA_REASON ,RATE) values (SYS_GUID()||'',"+
    				...
    				")";
    	    rs.executeSql(sql); 
            bb.writeLog("插入 MD_SD_GPM_RANG_AR:" + sql);
        //}
        //else{
