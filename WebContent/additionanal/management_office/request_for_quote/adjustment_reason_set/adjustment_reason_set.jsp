...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",adjustment_reason_setupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="adjustment_reason_set.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='adjustment_reason_set.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		            ...
		            String fromSql  = " MD_SD_FPA_REASON a ";
...
//修改
function doEdit(id){
	document.frmmain.action = "adjustment_reason_setupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("adjustment_reason_setexcel.jsp");
	if(rvalue > 0){
		window.location.href = "adjustment_reason_set.jsp";
	}
}
function doDownload(){
	window.location.href='adjustment_reason_set.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="adjustment_reason_setoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
