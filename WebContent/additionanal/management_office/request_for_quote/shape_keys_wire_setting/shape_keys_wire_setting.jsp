...
String MTRL_COST = Util.null2String(request.getParameter("MTRL_COST"));
...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",shape_keys_wire_settingupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="shape_keys_wire_setting.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='shape_keys_wire_setting.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		        	<%
		            String backfields = " a.id,b.name,a.QTTN_PKG_FK,MTRL_COST,MAINTENANCE_COST,LABOUR,POWER,DEPR,OTHER";
		            String fromSql  = " MD_SD_COST_WIRE a left join md_sd_wire b on A.WIRE_FK =B.ID";
		            ...
		            tableString =" <table instanceid=\"contract\" tabletype=\"none\" pagesize=\""+perpage+"\" >"+
		                         ...
		                         "		<head>"+
								"			<col width=\"10%\" text=\"键合丝\" column=\"name\" orderkey=\"name\"  />"+
								 "			<col width=\"10%\" text=\"外形\" column=\"QTTN_PKG_FK\" orderkey=\"QTTN_PKG_FK\"  />"+
								 "			<col width=\"5%\" text=\"材料费\" column=\"MTRL_COST\" orderkey=\"MTRL_COST\"  />"+
								 "			<col width=\"5%\" text=\"维修费\" column=\"MAINTENANCE_COST\" orderkey=\"MAINTENANCE_COST\"  />"+
								"			<col width=\"5%\" text=\"人工费\" column=\"LABOUR\" orderkey=\"LABOUR\"  />"+
								"			<col width=\"5%\" text=\"动力费\" column=\"POWER\" orderkey=\"POWER\"  />"+
								"			<col width=\"5%\" text=\"折旧费\" column=\"DEPR\" orderkey=\"DEPR\"  />"+
...
//修改
function doEdit(id){
	document.frmmain.action = "shape_keys_wire_settingupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("shape_keys_wire_settingexcel.jsp");
	if(rvalue > 0){
		window.location.href = "shape_keys_wire_setting.jsp";
	}
}
function doDownload(){
	window.location.href='shape_keys_wire_setting.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="shape_keys_wire_settingoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
