...
<%
	...
	while( true ) {
        ...
        //rs.executeSql(sql);
        //if(!rs.next()){
        sql="select id from MD_SD_COST_MTRL_TYPE where full_name='"+c3+"'";
        rs.executeSql(sql); 
        while(rs.next()){
        	String COST_MTRL_TYPE_FK =rs.getString("id");
        }
        
        	sql = "insert into MD_SD_COST_ELEMENT (id,name,sort,full_name) values (SYS_GUID()||'',"+
    				"'" + c2 + "'," +
    				"'" + COST_MTRL_TYPE_FK + "'" +
    				")";
    	    rs.executeSql(sql); 
            bb.writeLog("插入 MD_SD_COST_ELEMENT:" + sql);
        //}
        //else{
