...
int nodeidT5 = BillUtil.getNodeId(workflowid, "T5-MaterialSapDelivery");
...
<%}%>
 //根据标准工艺路线带出明细信息
	 $("#field"+bzgylx).bindPropertyChange(function(){
		 ...
		 var gx = '';
		 jQuery.ajax({
			 url: "/interface/additional/aoatcreate/aoatcreate_hf/WF1186ajaxaction.jsp",
			 ...
			 success: function (result) {
				...
				for(var i=0;i<result.length;i++){
					...
					<%if(nodeid == nodeidA1){%>
						...
						gx = result[i].oper_fk;
						jQuery.ajax({
							 url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118607ajaxaction.jsp",
					...
					<%if(nodeid == nodeidT1){%>
						...
						//带出工作中心 T1
						jQuery.ajax({
							url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118605ajaxaction.jsp",
							data: {type: 2, fzwx: fzwx,gxms: gxms,khh: khh},
							cache: false,
							type: "post",
							dataType: "json",
							async: false,
...
$("button[name=addbutton1]").click(function(){
	...
	var gx = jQuery("#field"+ gylx_oper +"_" + lineNumber + "span > span > a").html();
		if(gx != ""){
			jQuery.ajax({
				url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118603ajaxaction.jsp",
...
$("input[id^=field"+ xp_xplh +"_]").bindPropertyChange(function(e){
	jQuery.ajax({
		url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118602ajaxaction.jsp",
		data: {type: 1, gylxdm: e.value},
		cache: false,
		type: "post",
		dataType: "json",
...
$("input[id^=field"+ wlqd_gxxmh_a6 +"_]").bindPropertyChange(function(e){
	...
	jQuery.ajax({
		 url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118601ajaxaction.jsp",
		 data: {type: 1, gylxdm: e.value},
		 cache: false,
		 type: "post",
		 dataType: "json",
...
/*				$("input[id^=field"+ wlqd_operdesc +"_]").each(function(index) {
		...
		jQuery.ajax({
			 ...
			 success: function (result) {
				 ...
				 if(lastoper != oper){
					 jQuery.ajax({
						 url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118610ajaxaction.jsp",
						 //data: {type: 1,gylxdm: $("#field"+ bzgylx).val()},
...
/*				$("input[id^=field"+ wlqd_operdesc +"_]").each(function(index) {
		...
		jQuery.ajax({
			 ...
			 success: function (result) {
				 ...
				 if(lastoper != oper){
					 jQuery.ajax({
						 url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118609ajaxaction.jsp",
						 data: {type: 1, requestid: requestid , formid: <%=formid%>},
						 cache: false,
						 type: "post",
						 dataType: "json",
...
/*					$("#field"+ wlqd_gxxmh_a34 +"_"+ xzid).val((len+1)*10);
	...
	var khh =  $("#field"+ Customer).val();
	jQuery.ajax({
		 url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118608ajaxaction.jsp",
...
//主数据申请T 分支合并 2458 
<%if(nodeid == nodeidA31 ||nodeid == nodeidA32 ||nodeid == nodeidA34 ||nodeid == nodeidA41 ||nodeid == nodeidA42 ||nodeid == nodeidA44 ||nodeid == nodeidA6 ||nodeid == nodeidA7 ||nodeid == nodeidAgd||nodeid == nodeidT4 ||nodeid == nodeidT32 ||nodeid == nodeidT5){%>
...
<%if(nodeid == nodeidA1  ||nodeid == nodeidT1){%>
	$("#field" + bzgylx).bindPropertyChange(function(e){
		jQuery.ajax({
			url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118606ajaxaction.jsp",
			data: {type: 1, gylxdm: e.value},
			cache: false,
			type: "post",
			dataType: "json",
...
//封装信息判断品名描述
	$("#field"+ pmms1).blur(function(e){
		...
		jQuery.ajax({
			url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118604ajaxaction.jsp",
...
//});
  ...
  //T1根据品名描述带出 TestTime IndexTime SiteQty 
   ...
   var T_pmms = $("#field"+ pmms1).val();
	   jQuery.ajax({
	 		url: "/interface/additional/aoatcreate/aoatcreate_hf/WF118611ajaxaction.jsp",
