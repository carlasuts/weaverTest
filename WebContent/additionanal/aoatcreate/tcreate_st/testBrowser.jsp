	...
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'  attributes="{'groupSHBtnDisplay':'none'}" >
		...
		<wea:item>
			<brow:browser name="oper_fk" viewType="0" hasBrowser="true" hasAdd="false"
						  browserUrl='/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.jsgx_st_t'
						  isMustInput="1"
						  isSingle="false"
						  hasInput="true"
...
String fromSql  = " (select to_char(c.id) as id,  g.ITEMNO,g.operadd,c.mat_id ,c.mat_desc, c.mat_type   ,c.storage_location \n" +
		"FROM obomsetmat_st c ,   \n" +
		"(select f.ITEMNO,f.operadd ,mp.oper_grp_4 from formtable_main_150 e,formtable_main_150_dt2 f ,mwipoprdef mp  \n" +
		...
		"(select f.ITEMNO,f.operadd from formtable_main_150 e,formtable_main_150_dt2 f  \n" +
		"  where e.requestid =  " + requestid + " \n" +
		"  and  f.mainid = e.id  ) g  \n" +
		...
		"  and js.rolesmark = wl.oper_role_fk )) ";
e1.writeLog(fromSql);
	tableString =   " <table instanceid=\"db_list3\" tabletype=\"checkbox\" pagesize=\"6\" >"+ //指定分页条数和初始化id以及是否有复选框
			...
			" sqlorderby=\""+orderby+"\"   sqlprimarykey=\"id\" sqlsortway=\"asc\" sqlisdistinct=\"false\" />"+
                ...
                "           <col width=\"60%\"  text=\"料号\" column=\"mat_id\" orderkey=\"mat_id\"   />"+
                "           <col width=\"20%\"  text=\"工序\" column=\"operadd\" orderkey=\"operadd\"   />"+
                "       </head>"+
                " </table>";
%>
...
jQuery(document).ready(function(){
	...
	if(jQuery("#isfirst").val()==''){
		jQuery("#cust_id").val(parentWin.document.getElementById("field31032").value);
		jQuery("#pkld").val(parentWin.document.getElementById("field20909").value);
