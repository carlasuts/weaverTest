	...
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'  attributes="{'groupSHBtnDisplay':'none'}" >
		...
		<wea:item>
			<brow:browser name="oper_fk" viewType="0" hasBrowser="true" hasAdd="false"
						  browserUrl='/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.jsgx_t'
						  isMustInput="1"
						  isSingle="false"
						  hasInput="true"
...
String fromSql  = " (select to_char(c.id) as id,  g.ITEMNO,g.operadd,c.mat_id ,c.mat_desc, c.mat_type   ,c.storage_location \n" +
		"FROM obomsetmat_cc c ,   \n" +
		"(select f.ITEMNO,f.operadd ,mp.oper_grp_4 from formtable_main_144 e,formtable_main_144_dt2 f ,mwipoprdef mp  \n" +
		...
		"(select f.ITEMNO,f.operadd from formtable_main_144 e,formtable_main_144_dt2 f  \n" +
		"  where e.requestid =  " + requestid + " \n" +
		"  and  f.mainid = e.id  ) g  \n" +
...
jQuery(document).ready(function(){
	...
	if(jQuery("#isfirst").val()==''){
		jQuery("#cust_id").val(parentWin.document.getElementById("field18243").value);
		jQuery("#pkld").val(parentWin.document.getElementById("field18312").value);
