...
if(type.equals("1")){
	...
	sql = "select * from MDM_UPH_RULE_cc where pkg_outline_fk = '"+fzwx+"' and oper_fk = '"+gx+"'";
	rs.executeSql(sql);
	result +="[";
	while(rs.next()){
...
if(type.equals("2")){
	...
	if(rs.getCounts()>0){
		while(rs.next()){
		result = rs.getString("SITE_QTY") + "," +rs.getString("INDEX_TIME") + ","+rs.getString("TEST_TIME") + ","+rs.getString("CSJ") + ","+rs.getString("CSLC");
		//sb.app
