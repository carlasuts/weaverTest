...
//获取节点ID
int nodeidA1 = BillUtil.getNodeId(workflowid, "A1-AssyRouterHandler");
int nodeidA2 = BillUtil.getNodeId(workflowid, "A2-AssyRouterAudit");
int nodeidDraft = BillUtil.getNodeId(workflowid, "Draft");
int nodeidA31 = BillUtil.getNodeId(workflowid, "A3-1-AssyPackHandler");
int nodeidA32 = BillUtil.getNodeId(workflowid, "A3-2-AssyWireBondHandler");
int nodeidA42 = BillUtil.getNodeId(workflowid, "A4-2-AssyWireBondAudit");
int nodeidA34 = BillUtil.getNodeId(workflowid, "A3-4-AssyTechHandler");
int nodeidA41 = BillUtil.getNodeId(workflowid, "A4-1-AssyPackAudit");
int nodeidA44 = BillUtil.getNodeId(workflowid, "A4-4-AssyTechAudit");
int nodeidA6 = BillUtil.getNodeId(workflowid, "A6-AssyPrencheck");
int nodeidA61 = BillUtil.getNodeId(workflowid, "A6-1-BOMcheck");
int nodeidAgd = BillUtil.getNodeId(workflowid, "归档");
int nodeidA35 = BillUtil.getNodeId(workflowid, "A3-5-AssyWorkCenterHandler");
int nodeidA5 = BillUtil.getNodeId(workflowid, "A5-AssyUPHHandler");
int nodeidA7 = BillUtil.getNodeId(workflowid, "A7-MaterialSapDelivery");
...
//测试期别
String TestStage = BillUtil.getLabelId(formid,0,"TestStage"); 
//包装方式 PackageType
...
<%}%>
 //根据标准工艺路线带出明细信息
	 $("#field"+bzgylx).bindPropertyChange(function(){
		 ...
		 var gx = '';
		 jQuery.ajax({
			 url: "/interface/additional/aoatcreate/aoatcreate_st/WF1186ajaxaction.jsp",
			 ...
			 success: function (result) {
				...
				for(var i=0;i<result.length;i++){
					...
					<%if(nodeid == nodeidA1){%>
						...
						gx = result[i].oper_fk;
						jQuery.ajax({
							 url: "/interface/additional/aoatcreate/aoatcreate_st/WF118607ajaxaction.jsp",
					...
					<%if(nodeid == nodeidT1){%>
						...
						//带出工作中心 T1
						jQuery.ajax({
							url: "/interface/additional/aoatcreate/aoatcreate_st/WF118605ajaxaction.jsp",
							data: {type: 2, khh: khh,gxms: gxms},
							cache: false,
							type: "post",
							dataType: "json",
							async: false,
...
$("button[name=addbutton1]").click(function(){
	...
	var gx = jQuery("#field"+ gylx_oper +"_" + lineNumber + "span > span > a").html();
		if(gx != ""){
			jQuery.ajax({
				url: "/interface/additional/aoatcreate/aoatcreate_st/WF118603ajaxaction.jsp",
...
<%if(nodeid == nodeidDraft ||nodeid == nodeidT1){%>
$("input[id^=field"+ xp_xplh +"_]").bindPropertyChange(function(e){
	jQuery.ajax({
		url: "/interface/additional/aoatcreate/aoatcreate_st/WF118602ajaxaction.jsp",
		data: {type: 1, gylxdm: e.value},
		cache: false,
		type: "post",
		dataType: "json",
...
$("input[id^=field"+ wlqd_gxxmh_a6 +"_]").bindPropertyChange(function(e){
	...
	jQuery.ajax({
		 url: "/interface/additional/aoatcreate/aoatcreate_st/WF118601ajaxaction.jsp",
		 data: {type: 1, gylxdm: e.value},
		 cache: false,
		 type: "post",
		 dataType: "json",
...
/*				$("input[id^=field"+ wlqd_operdesc +"_]").each(function(index) {
		...
		jQuery.ajax({
			 ...
			 success: function (result) {
				 ...
				 if(lastoper != oper){
					 jQuery.ajax({
						 url: "/interface/additional/aoatcreate/aoatcreate_st/WF118610ajaxaction.jsp",
						 //data: {type: 1,gylxdm: $("#field"+ bzgylx).val()},
...
/*				$("input[id^=field"+ wlqd_operdesc +"_]").each(function(index) {
		...
		jQuery.ajax({
			 ...
			 success: function (result) {
				 ...
				 if(lastoper != oper){
					 jQuery.ajax({
						 url: "/interface/additional/aoatcreate/aoatcreate_st/WF118609ajaxaction.jsp",
						 data: {type: 1, requestid: requestid , formid: <%=formid%>},
						 cache: false,
						 type: "post",
						 dataType: "json",
...
/*					$("#field"+ wlqd_gxxmh_a34 +"_"+ xzid).val((len+1)*10);
	...
	var khh =  $("#field"+ Customer).val();
	jQuery.ajax({
		 url: "/interface/additional/aoatcreate/aoatcreate_st/WF118608ajaxaction.jsp",
...
//主数据申请T 分支合并 2458 
<%if(nodeid == nodeidA31 ||nodeid == nodeidA32 ||nodeid == nodeidA34 ||nodeid == nodeidA41 ||nodeid == nodeidA42 ||nodeid == nodeidA44 ||nodeid == nodeidA6 ||nodeid == nodeidA7 ||nodeid == nodeidAgd||nodeid == nodeidT4 ||nodeid == nodeidT32 || nodeid == nodeidA61){%>
...
<%if(nodeid == nodeidA34 || nodeid == nodeidA44){%>
	$("input[id^=field"+ wlqd_gxxmh_a34 +"_]").each(function(index){
		var tmp = this.id.split("_")[1];
...
<%if(nodeid == nodeidA1  ||nodeid == nodeidT1){%>
	$("#field" + bzgylx).bindPropertyChange(function(e){
		jQuery.ajax({
			url: "/interface/additional/aoatcreate/aoatcreate_st/WF118606ajaxaction.jsp",
			data: {type: 1, gylxdm: e.value},
			cache: false,
			type: "post",
			dataType: "json",
...
//封装信息判断品名描述
	$("#field"+ pmms1).blur(function(e){
		...
		jQuery.ajax({
			url: "/interface/additional/aoatcreate/aoatcreate_st/WF118604ajaxaction.jsp",
  ...
  //T1根据品名描述带出 TestTime IndexTime SiteQty ,csj ,cslc
   <%if(nodeid == nodeidT1){%>
   var T_pmms = $("#field"+ pmms1).val();
	   jQuery.ajax({
	 		url: "/interface/additional/aoatcreate/aoatcreate_st/WF118611ajaxaction.jsp",
...
<%if(nodeid == nodeidDraft){%>
	...
	//生产类型为AT，测试期别默认210
/* 	$("#field" + sclx1).bindPropertyChange(function(e){
		var options=$("#field"+ sclx1 +" option:selected");
		var sclx = options.text();
		if(sclx == "AT"){
			$("#field" + TestStage).val("210");
		}else{
			$("#field" + TestStage).val("");
		}
	}); */
