...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",gross_margin_nucleus_of_permissionsupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="gross_margin_nucleus_of_permissions.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='gross_margin_nucleus_of_permissions.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		        	<%
		            String backfields = " a.id,a.name,B.FULL_NAME,A.RATE,A.SORT";
		            String fromSql  = " MD_SD_GPM_AR a left join MD_APPROVEL_LEVEL b on A.APPROVEL_LEVEL_FK =b.id";
		            ...
		            tableString =" <table instanceid=\"contract\" tabletype=\"none\" pagesize=\""+perpage+"\" >"+
								...
								"			<col width=\"7%\" text=\"名称\" column=\"name\" orderkey=\"name\"  />"+
								 "			<col width=\"7%\" text=\"核准权限\" column=\"FULL_NAME\" orderkey=\"FULL_NAME\"  />"+
								 "			<col width=\"7%\" text=\"系数\" column=\"RATE\" orderkey=\"RATE\"  />"+
...
//修改
function doEdit(id){
	document.frmmain.action = "gross_margin_nucleus_of_permissionsupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("gross_margin_nucleus_of_permissionsexcel.jsp");
	if(rvalue > 0){
		window.location.href = "gross_margin_nucleus_of_permissions.jsp";
	}
}
function doDownload(){
	window.location.href='gross_margin_nucleus_of_permissions.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="gross_margin_nucleus_of_permissionsoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
