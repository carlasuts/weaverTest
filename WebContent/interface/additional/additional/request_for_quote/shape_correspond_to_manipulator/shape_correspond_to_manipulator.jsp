...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",shape_correspond_to_manipulatorupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="shape_correspond_to_manipulator.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='shape_correspond_to_manipulator.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		        	<%
		            String backfields = " a.id,b.name,A.QTTN_PKG_FK";
		            String fromSql  = " MD_SD_QTTN_PKG_HANDLER a left join MD_HANDLER b  on A.HANDLER_FK =b.id ";
		            ...
		            tableString =" <table instanceid=\"contract\" tabletype=\"none\" pagesize=\""+perpage+"\" >"+
		                         ...
		                         "		<head>"+
								"			<col width=\"15%\" text=\"机械手\" column=\"name\" orderkey=\"name\"  />"+
...
//修改
function doEdit(id){
	document.frmmain.action = "shape_correspond_to_manipulatorupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("shape_correspond_to_manipulatorexcel.jsp");
	if(rvalue > 0){
		window.location.href = "shape_correspond_to_manipulator.jsp";
	}
}
function doDownload(){
	window.location.href='shape_correspond_to_manipulator.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="shape_correspond_to_manipulatoroperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
