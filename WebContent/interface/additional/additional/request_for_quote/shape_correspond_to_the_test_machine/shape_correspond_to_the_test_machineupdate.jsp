...
<%
	...
	String TESTER_FK = "",name = "",QTTN_PKG_FK = "";

	RecordSet rs = new RecordSet();
	if(!id.equals("")){
		rs.executeSql("select a.TESTER_FK,b.name,A.QTTN_PKG_FK  from MD_SD_QTTN_PKG_TESTER a left join MD_TESTER b on A.TESTER_FK = B.ID where a.id ='"+id+"'");
		if(rs.next()){
...
<BODY>
	...
	<%
		...
		//返回
		RCMenu += "{" + SystemEnv.getHtmlLabelName(1290, user.getLanguage()) + ",javascript:location.href='shape_correspond_to_the_test_machine.jsp',_self} ";
		RCMenuHeight += RCMenuHeightStep;
	%>
	...
	<table width=100% height=100% border="0" cellspacing="0"
		...
		<tr>
			<td></td>
			<td valign="top">
				<FORM id=weaver NAME=frmmain STYLE="margin-bottom: 0" action="shape_correspond_to_the_test_machineoperation.jsp" method=post>
