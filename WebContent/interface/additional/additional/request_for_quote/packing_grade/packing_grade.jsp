...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",packing_gradeupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="packing_grade.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='packing_grade.xls';">-->
			</td>
		</TR>
		...
		<TABLE width="100%">
			<tr> 
				<td valign="top">
		            ...
		            String fromSql  = " MD_SD_PACKING_GRADE ";
...
//修改
function doEdit(id){
	document.frmmain.action = "packing_gradeupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("packing_gradeexcel.jsp");
	if(rvalue > 0){
		window.location.href = "packing_grade.jsp";
	}
}
function doDownload(){
	window.location.href='packing_grade.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="packing_gradeoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
