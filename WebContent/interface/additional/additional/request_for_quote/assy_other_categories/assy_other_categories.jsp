...
<%
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(82,user.getLanguage())+",assy_other_categoriesupdate.jsp,_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
...
<tr>
	...
	<td valign="top">
		...
		<tr>
		<td valign="top" >
		<FORM id=weaver NAME=frmmain STYLE="margin-bottom:0" action="assy_other_categories.jsp" method=post>
		...
		<TR>
			...
			<input type="button" value="模板下载" onclick="javascript:window.location.href='assy_other_categories.xls';"></td>
		</TR>
		...
		<TABLE width="100%">
			<tr>
				<td valign="top">
		        	<%
		            String backfields = " a.id,a.NAME,a.SORT,b.FULL_NAME ";
		            String fromSql  = " MD_SD_COST_ELEMENT a left join MD_SD_COST_MTRL_TYPE b on A.COST_MTRL_TYPE_FK = b.id ";
		            String sqlWhere = sqlwhere +" and PARENT_FK ='M8A8HPT2CVYW0ZOYHQ96JMM7QMKUSGO8' ";
		            String orderby = " a.id " ;
		            String tableString = "";
		            tableString =" <table instanceid=\"contract\" tabletype=\"none\" pagesize=\""+perpage+"\" >"+
								...
								"			<col width=\"10%\" text=\"名称\" column=\"name\" orderkey=\"name\"  />"+
								 "			<col width=\"10%\" text=\"材料类型\" column=\"FULL_NAME\" orderkey=\"FULL_NAME\"  />"+
...
//修改
function doEdit(id){
	document.frmmain.action = "assy_other_categoriesupdate.jsp?type=modi&id="+id;
	document.frmmain.submit();
}
//导入
function doImportExcel(){
	var rvalue = window.showModalDialog("assy_other_categoriesexcel.jsp");
	if(rvalue > 0){
		window.location.href = "assy_other_categories.jsp";
	}
}
function doDownload(){
	window.location.href='assy_other_categories.xls';
}
function doDel(id){
	var isdel = confirm("真的要删除吗?");
	if(isdel){
		document.frmmain.action="assy_other_categoriesoperation.jsp?type=del&id="+id;
		document.frmmain.submit();	
	}
}
</script>
