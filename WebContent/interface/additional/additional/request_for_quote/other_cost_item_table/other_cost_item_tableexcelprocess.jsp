...
<%
	...
	while( true ) {
        ...
        sql ="select id from MD_SD_COST_ELEMENT where name= " +c3+"'";
        rs.executeSql(sql);
        while(rs.next()){
        	String COST_ELEMENT_FK =rs.getString("id");
        }
        
        	sql = "insert into MD_SD_COST_ASSY_OTHER (id,name,MD_SD_COST_ELEMENT,QTTN_PKG_FK) values (SYS_GUID()||'',"+
    				"'" + c2 + "'," +
    				"'" + COST_ELEMENT_FK + "'," +
    				"'" + c4 + "'" +
    				")";
    	    rs.executeSql(sql); 
            bb.writeLog("插入 MD_SD_COST_ASSY_OTHER:" + sql);
        //}
        //else{
