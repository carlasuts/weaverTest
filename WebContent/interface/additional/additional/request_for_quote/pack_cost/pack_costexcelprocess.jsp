...
<%
	...
	while( true ) {
        ...
        sql="select id from MD_SD_PACKING where name = '"+c3+"'";
        rs.executeSql(sql);
        while(rs.next()){
        	String PACKING_FK =rs.getString("id");
        }
        
        sql="select id from MD_SD_PACKING_GRADE where name = '"+c4+"'";
        rs.executeSql(sql);
        while(rs.next()){
        	String PACKING_GRADE_FK =rs.getString("id");
        }
        	...
        	sql = "insert into MD_SD_COST_PACKING (id,QTTN_PKG_FK,PACKING_FK,PACKING_GRADE_FK,MATERIAL,MAINTENANCE_COST,LABOUR,POWER,DEPR,OTHER) values (SYS_GUID()||'',"+
    				...
    				")";
    	    rs.executeSql(sql); 
            bb.writeLog("插入 MD_SD_COST_PACKING:" + sql);
        //}
        //else{
