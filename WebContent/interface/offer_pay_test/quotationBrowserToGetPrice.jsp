...
String uph =  Util.null2String(request.getParameter("uph"));
		...
		//划片实际成本
		BigDecimal scrib_actual_1 = scrib_1.multiply(base_scrib).multiply(chipnumber_no);
		BigDecimal scrib_actual_2 = scrib_2.multiply(base_scrib).multiply(chipnumber_no);
		BigDecimal scrib_actual_3 = scrib_3.multiply(base_scrib).multiply(chipnumber_no);
		BigDecimal scrib_actual_4 = scrib_4.multiply(base_scrib).multiply(chipnumber_no);
		BigDecimal scrib_actual_5 = scrib_5.multiply(base_scrib).multiply(chipnumber_no);
		BigDecimal scrib_actual_6 = scrib_6.multiply(base_scrib).multiply(chipnumber_no);
		BigDecimal scrib_actual_7 = scrib_7.multiply(base_scrib).multiply(chipnumber_no);
		//磨片标准成本
		...
		//磨片实际成本
		BigDecimal grind_actual_1 = grind_1.multiply(base_grind).multiply(chipnumber_no);
		BigDecimal grind_actual_2 = grind_2.multiply(base_grind).multiply(chipnumber_no);
		BigDecimal grind_actual_3 = grind_3.multiply(base_grind).multiply(chipnumber_no);
		BigDecimal grind_actual_4 = grind_4.multiply(base_grind).multiply(chipnumber_no);
		BigDecimal grind_actual_5 = grind_5.multiply(base_grind).multiply(chipnumber_no);
		BigDecimal grind_actual_6 = grind_6.multiply(base_grind).multiply(chipnumber_no);
		BigDecimal grind_actual_7 = grind_7.multiply(base_grind).multiply(chipnumber_no);
		//装片标准成本
		...
		sql ="SELECT  a.QTTN_PKG_FK,C.FULL_NAME,b.name,MATERIAL,MAINTENANCE_COST,LABOUR,POWER,DEPR,OTHER,(MATERIAL+MAINTENANCE_COST+LABOUR+POWER+DEPR+OTHER) AS TOTAL FROM MD_SD_COST_DIE a left join MD_SD_MOUNTING_MTRL_SPEC b on A.MTRL_SPEC=B.ID left join MD_SD_COST_DIE_CAT c on A.COST_DIE_CAT_FK =C.ID   where QTTN_PKG_FK = '"+shape+"' AND C.FULL_NAME ='装片' and b.name ='"+mat+"'";
		...
		//装片实际成本
		BigDecimal Load_actual_1 = Load_1.multiply(base_Load).multiply(chipnumber_no);
		BigDecimal Load_actual_2 = Load_2.multiply(base_Load).multiply(chipnumber_no);
		BigDecimal Load_actual_3 = Load_3.multiply(base_Load).multiply(chipnumber_no);
		BigDecimal Load_actual_4 = Load_4.multiply(base_Load).multiply(chipnumber_no);
		BigDecimal Load_actual_5 = Load_5.multiply(base_Load).multiply(chipnumber_no);
		BigDecimal Load_actual_6 = Load_6.multiply(base_Load).multiply(chipnumber_no);
		BigDecimal Load_actual_7 = Load_7.multiply(base_Load).multiply(chipnumber_no);
		//芯片实际成本
		...
		BigDecimal Load_other_true_2 = Load_other_2.multiply(chipnumber_no);
		BigDecimal Load_other_true_3 = Load_other_3.multiply(chipnumber_no);
		BigDecimal Load_other_true_4 = Load_other_4.multiply(chipnumber_no);
		BigDecimal Load_other_true_5 = Load_other_5.multiply(chipnumber_no);
		BigDecimal Load_other_true_6 = Load_other_6.multiply(chipnumber_no);
		BigDecimal Load_other_true_7 = Load_other_7.multiply(chipnumber_no);
		...
		bs.writeLog("sql外形键丝设定   取值 单丝长度 标准丝数"+sql);
 		...
 		BigDecimal bonding_mat_cost =current_price_bonding.add((current_price_category.subtract(price_category)).divide(price_step_bonding,4,BigDecimal.ROUND_HALF_UP).multiply(price_diff_bonding)).multiply(wire_length).multiply(silk_standard);
		...
		//键合实际工费 
				...
				BigDecimal bonded_labour_true_1 = bonded_labour_fee_1.divide(wire_qty,4,BigDecimal.ROUND_HALF_UP).multiply(silk_standard);
				BigDecimal bonded_labour_true_2 = bonded_labour_fee_2.divide(wire_qty,4,BigDecimal.ROUND_HALF_UP).multiply(silk_standard);
				BigDecimal bonded_labour_true_3 = bonded_labour_fee_3.divide(wire_qty,4,BigDecimal.ROUND_HALF_UP).multiply(silk_standard);
				BigDecimal bonded_labour_true_4 = bonded_labour_fee_4.divide(wire_qty,4,BigDecimal.ROUND_HALF_UP).multiply(silk_standard);
				BigDecimal bonded_labour_true_5 = bonded_labour_fee_5.divide(wire_qty,4,BigDecimal.ROUND_HALF_UP).multiply(silk_standard);
				BigDecimal bonded_labour_true_6 = bonded_labour_fee_6.divide(wire_qty,4,BigDecimal.ROUND_HALF_UP).multiply(silk_standard);
				BigDecimal bonded_labour_true_7 = bonded_labour_fee_7.divide(wire_qty,4,BigDecimal.ROUND_HALF_UP).multiply(silk_standard);
		...
		bs.writeLog("sql框架工艺"+sql);
			...
			BigDecimal frametechnology_ftech_true = frametechnology_fees_1.multiply(rate_ftech);
		...
		sql ="select b.name,B.QTTN_PKG_FK,A.FULL_NAME,A.NAME,A.SORT,A.CURRENCY_FK, MATERIALCOST,MAINTENANCE_COST,UC,LABOUR,POWER,DEPR,OTHER,(MATERIALCOST+MAINTENANCE_COST+LABOUR+POWER+DEPR+OTHER) as TOTAL from MD_SD_COST_ASSY_OTHER_OPTION a left join  MD_SD_COST_ASSY_OTHER b on a.COST_ASSY_OTHER_FK =b.id where QTTN_PKG_FK ='"+ shape +"'  and a.NAME ='"+moldingcompound+"'";
			rs.executeSql(sql);
			...
			BigDecimal moldingcompound_mat_true = moldingcompound_fees_1.multiply(rate_emat);
		...
		//组装 各项金额
		BigDecimal assembly_mat_cost =scrib_actual_1.add(grind_actual_1).add(Load_actual_1).add(bonding_mat_cost).add(bonded_labour_true_1).add(frametechnology_ftech_true).add(moldingcompound_mat_true).add(other_fees_1).add(print_fees_1);//组装材料费用
		...
		sql ="select MAINTENANCE_COST,LABOUR,POWER,DEPR,OTHER ,(MAINTENANCE_COST+LABOUR+POWER+DEPR+OTHER) as TOTAL from MD_SD_TESTER_COST a left join MD_TESTER b on A.TESTER_FK =B.ID where b.name ='"+ tester +"'";
 		...
 		sql ="select A.INDEX_TIME,MAINTENANCE_COST,LABOUR,POWER,DEPR,OTHER ,(MAINTENANCE_COST+LABOUR+POWER+DEPR+OTHER)  as TOTAL  from MD_SD_HANDLER_COST a left join MD_HANDLER b on A.HANDLER_FK=B.ID where b.name ='"+ manipulator +"'";
			...
			}else if (quotationmethod.equals("1")){
				 test_uph = new BigDecimal(uphformula);
				 test_mat_cost =new BigDecimal(0);
				 test_maintenance_cost =testing_machine_cost_2.add(hander_machine_cost_2).divide(test_uph,4,BigDecimal.ROUND_HALF_UP).multiply( new BigDecimal(1000));
				 test_labour_cost  =testing_machine_cost_3.add(hander_machine_cost_3).divide(test_uph,4,BigDecimal.ROUND_HALF_UP).multiply( new BigDecimal(1000));
				 test_power_cost  =testing_machine_cost_4.add(hander_machine_cost_4).divide(test_uph,4,BigDecimal.ROUND_HALF_UP).multiply( new BigDecimal(1000));
				 test_depr_cost  =testing_machine_cost_5.add(hander_machine_cost_5).divide(test_uph,4,BigDecimal.ROUND_HALF_UP).multiply( new BigDecimal(1000));
				 test_other_cost  =testing_machine_cost_6.add(hander_machine_cost_6).divide(test_uph,4,BigDecimal.ROUND_HALF_UP).multiply( new BigDecimal(1000));
				 test_total_cost2  =testing_machine_cost_7.add(hander_machine_cost_7).divide(test_uph,4,BigDecimal.ROUND_HALF_UP).multiply( new BigDecimal(1000));
			...
			//加一个芯片  加一根丝  成本
				 BigDecimal advice_chip_cose =cost_of_one.add(cost_of_two).divide(new BigDecimal(1).subtract(GPM),4,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(1).add(rate));
