...
<%
	...
	String titlename = "客户CE负责人员配置(合肥)";
	String needfav = "1";
	String needhelp = "";
%>
<BODY>
	...
	<%
		...
		RCMenu += "{" + SystemEnv.getHtmlLabelName(82, user.getLanguage()) + ",oinrwcedopupdate.jsp,_self} ";
		RCMenuHeight += RCMenuHeightStep;
					...
					<tr>
						<td valign="top">
							<FORM id=weaver NAME=frmmain STYLE="margin-bottom: 0"
								action="oinrwcedop.jsp" method=post>
								<table width=100% class=ViewForm>
									...
									<TR>
										...
										<td NOWRAP width='25%'><input type="button" value="搜索"
											...
											onclick="javascript:window.location.href='oinrwcedop.xls';">&nbsp;&nbsp;
									...
									<tr>
										<td valign="top">
											<%
												...
												String fromSql = " oinrwcedop_hf b ";
	...
	<script language=javascript>
		...
		//修改
		function doEdit(id) {
			document.frmmain.action = "oinrwcedopupdate.jsp?type=modi&id=" + id;
			document.frmmain.submit();
		}
		//导入
		function doImportExcel() {
			var rvalue = window.showModalDialog("oinrwcedopexcel.jsp");
			if (rvalue > 0) {
				window.location.href = "oinrwcedop.jsp";
			}
		}
		function doDownload() {
			window.location.href = 'oinrwcedop.xls';
		}
		function doDel(id) {
			var isdel = confirm("真的要删除吗?");
			if (isdel) {
				document.frmmain.action = "oinrwcedopoperation.jsp?type=del&id="
						+ id;
				document.frmmain.submit();
			}
		}
		// 新增
		function doInsert() {
			document.frmmain.action = "oinrwcedopupdate.jsp?type=ins";
			document.frmmain.submit();
		}
	</script>
</body>
