...
<%
	...
	while (true) {
		...
		//重复数据更新
		sql = "SELECT * FROM OINRWCPDOP_HF WHERE CUST_ID = '" + c1 + "' AND WORK_CODE = '" + c3 + "' ";
		rs1.executeSql(sql);
		if (!rs1.next()) {
			sql = "INSERT INTO OINRWCPDOP_HF (CUST_ID, NAME, WORK_CODE) VALUES ('" + c1 + "','" + c2 + "','" + c3
					+ "')";
			rs.executeSql(sql);
		} else {
			sql = "UPDATE OINRWCPDOP_HF SET (NAME, WORK_CODE) = ('" + c2 + "','" + c3 + "') where cust_id = '" + c1
					+ "'";
			rs2.executeSql(sql);
		}
	}
