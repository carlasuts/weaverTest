...
<%
	...
	while (true) {
		...
		//重复数据更新
		sql = "SELECT * FROM OINRWSODOP_ST WHERE CUST_ID = '" + c1 + "' AND WORK_CODE = '" + c3 + "' ";
		rs1.executeSql(sql);
		if (!rs1.next()) {
			sql = "INSERT INTO OINRWSODOP_ST (CUST_ID, NAME, WORK_CODE) VALUES ('" + c1 + "','" + c2 + "','" + c3
					+ "')";
			rs.executeSql(sql);
			bb.writeLog("OINRWSODOP_ST insertsql:" + sql);
		} else {
			sql =  "UPDATE OINRWSODOP_ST SET (NAME, WORK_CODE) = ('" + c2 + "','" + c3 + "') where cust_id = '" + c1
					+ "'";
			rs2.executeSql(sql);
