...
<%
	...
	String titlename = "客户CE负责人员配置(苏通)";
	String needfav = "1";
	String needhelp = "";
%>
<%
	...
	if (!id.equals("")) {
		rs.executeSql("select cust_id, name, work_code from oinrwcedop_st where id ='" + id + "'");
		if (rs.next()) {
...
<BODY>
	...
	<%
		...
		RCMenu += "{" + SystemEnv.getHtmlLabelName(1290, user.getLanguage())
				+ ",javascript:location.href='oinrwcedop.jsp',_self} ";
		RCMenuHeight += RCMenuHeightStep;
	%>
		...
		<tr>
			...
			<td valign="top">
				<FORM id=weaver NAME=frmmain STYLE="margin-bottom: 0"
					action="oinrwcedopoperation.jsp" method=post>
