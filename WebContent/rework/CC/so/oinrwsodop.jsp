...
<BODY>
	...
	<%
		...
		RCMenu += "{" + SystemEnv.getHtmlLabelName(82, user.getLanguage()) + ",oinrwsodopupdate.jsp,_self} ";
		RCMenuHeight += RCMenuHeightStep;
					...
					<tr>
						<td valign="top">
							<FORM id=weaver NAME=frmmain STYLE="margin-bottom: 0"
								action="oinrwsodop.jsp" method=post>
								<table width=100% class=ViewForm>
									...
									<TR>
										...
										<td NOWRAP width='25%'><input type="button" value="搜索"
											...
											onclick="javascript:window.location.href='oinrwsodop.xls';">&nbsp;&nbsp;
									...
									<tr>
										<td valign="top">
											<%
												...
												String fromSql = " oinrwsodop_cc b ";
	...
	<script language=javascript>
		...
		//修改
		function doEdit(id) {
			document.frmmain.action = "oinrwsodopupdate.jsp?type=modi&id=" + id;
			document.frmmain.submit();
		}
		//导入
		function doImportExcel() {
			var rvalue = window.showModalDialog("oinrwsodopexcel.jsp");
			if (rvalue > 0) {
				window.location.href = "oinrwsodop.jsp";
			}
		}
		function doDownload() {
			window.location.href = 'oinrwsodop.xls';
		}
		function doDel(id) {
			var isdel = confirm("真的要删除吗?");
			if (isdel) {
				document.frmmain.action = "oinrwsodopoperation.jsp?type=del&id="
						+ id;
				document.frmmain.submit();
			}
		}
		// 新增
		function doInsert() {
			document.frmmain.action = "oinrwsodopupdate.jsp?type=ins";
			document.frmmain.submit();
		}
	</script>
</body>
