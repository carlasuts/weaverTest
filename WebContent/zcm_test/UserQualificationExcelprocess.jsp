...
<%
	...
	StringBuilder sb = new StringBuilder();
     ...
     while( true ) {
		...
		//根据封装外形工序确定唯一
		sql = "select * from okeyuserinf where workcode = '" + c2+"'";
		rs1.executeSql(sql);
		create_time=df.format(System.currentTimeMillis());
		update_time=df.format(System.currentTimeMillis());
		
		if(!rs1.next()){
			sql = "insert into OKEYUSERINF (workcode,site,name,DEPART,create_time,update_time,TOTAL_REJECT_QTY,REJECTCODE1_QTY,REJECTCODE2_QTY,REJECTCODE3_QTY,REJECTCODE4_QTY, REJECTCODE5_QTY, REJECTCODE6_QTY) values ("+
			"'" + c2 + "'," +
			"'" + c3 + "'," +
			"'" + c4 + "'," +
			"'" + DEPART + "'," +
		...
		sql = "update OKEYUSERINF set site = '"+ c3 +"' ,name = '"+ c4 +"' ,DEPART = '"+ DEPART +"',TOTAL_REJECT_QTY = '"+ c5 +"',REJECTCODE1_QTY = '"+ c6 +"',REJECTCODE2_QTY= '"+ c7 +"',REJECTCODE3_QTY = '"+ c8 +"',REJECTCODE4_QTY = '"+ c9+"',REJECTCODE5_QTY = '"+ c10+"',REJECTCODE6_QTY = '"+ c11+"',update_time = '"+ update_time +"' where workcode = '" + c2+"'";
	    rs.executeSql(sql);
