...
String sqlwhere = " 1=1  and NAME = '其它' ";
	int ishead = 0;
...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="otherBrowser.jsp" method=post>
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'  attributes="{'groupSHBtnDisplay':'none'}" >
		<wea:item>其它</wea:item>
		<wea:item><input name="other" value='<%=other%>' class="InputStyle"></wea:item>
	</wea:group>
</wea:layout>
</div>
...
String tableString = "";
int perpage=10;
String backfields = " distinct other, QTTN_PKG_FK, NAME ";
String fromSql  = " (select a.FULL_NAME as other, b.QTTN_PKG_FK, b.NAME from MD_SD_COST_ASSY_OTHER_OPTION a left join  MD_SD_COST_ASSY_OTHER b on a.COST_ASSY_OTHER_FK =b.id) ";
	tableString =   " <table instanceid=\"db_list3\" tabletype=\"none\" pagesize=\""+perpage+"\"  >"+ //指定分页条数和初始化id以及是否有复选框
			...
			" sqlprimarykey=\"other\" sqlsortway=\"asc\" sqlisdistinct=\"false\" />"+
                "       <head>"+
                "           <col width=\"0%\" hide=\"true\"  text=\""+"other"+"\" column=\"other\"    />"+
	        	"           <col width=\"100%\"  text=\"其它\" column=\"other\" orderkey=\"other\"   />"+
                "       </head>"+
                " </table>";
%>
