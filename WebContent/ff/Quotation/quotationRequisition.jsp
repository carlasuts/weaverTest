	...
	<td valign="top">
		...
		<TABLE width="600%">
			<tr>
				<td valign="top">
		        	<%
					String backfields = " id, process_number, LASTNAME, createtime, State_form, Quotation_way, CUSTID, shape, product_type, Tradeway, currency, endtime, exchange_rate, " + 
										...
										" packagediscount, salesarea, businessgroup, On_demand, isDeal, failurereason, actuarialstarttime, actuarialendtime, addbondingcost, addbondingprice," + 
		            ...
		            String fromSql = " (select tb.id ,process_number, (select LASTNAME from HRMRESOURCE h where Createpersonnel = h.id) as LASTNAME, createtime, State_form, " +
										" (select SELECTNAME from (SELECT SELECTVALUE, SELECTNAME FROM workflow_selectitem " + 										
										" WHERE fieldid = (SELECT id FROM workflow_billfield WHERE billid = -141 AND fieldname = 'Quotation_way')) where SELECTVALUE = tb.Quotation_way) as Quotation_way, " +
										" CUSTID, shape, (select SELECTNAME from (SELECT SELECTVALUE, SELECTNAME FROM workflow_selectitem " +
										" WHERE fieldid = (SELECT id FROM workflow_billfield WHERE billid = -141 AND fieldname = 'product_type')) where SELECTVALUE = tb.product_type) as product_type, " +
										" (select SELECTNAME from (SELECT SELECTVALUE, SELECTNAME FROM workflow_selectitem " +
										" WHERE fieldid = (SELECT id FROM workflow_billfield WHERE billid = -141 AND fieldname = 'Tradeway')) where SELECTVALUE = tb.Tradeway) as Tradeway, " +
										" (select SELECTNAME from (SELECT SELECTVALUE, SELECTNAME FROM workflow_selectitem " +
										" WHERE fieldid = (SELECT id FROM workflow_billfield WHERE billid = -141 AND fieldname = 'currency')) where SELECTVALUE = tb.currency) as currency, " +
										" (select OPERATEDATE from workflow_requestbase a left join WORKFLOW_CURRENTOPERATOR b on A.REQUESTID = B.REQUESTID and A.CURRENTNODEID = B.NODEID " +
										" where a.REQUESTID = tb.REQUESTID) as endtime, " +
										...
										" test_tal_cost_thou, (select deal_price from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 1) as assembleprice, " +
										" (select deal_price from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 2) as packageprice, " +
										" (select deal_price from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 0 and Quotation_mode = 0) as thoutestprice, " +
										" (select gross_margin from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 1) as assemblegross, " +
										" (select gross_margin from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 2) as packagegross, " +
										" (select gross_margin from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 0 and Quotation_mode = 0) as thoutestgross, " +
										" (select round(((sum(suggested_price) - sum(deal_price)) / sum(suggested_price)), 3) from FORMTABLE_MAIN_141_DT1 where mainid = TB.id) as totaldiscount, " +
										" (select discount from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 1) as assemblediscount, " +
										" (select discount from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 0 and Quotation_mode = 0) as thoutestdiscount, " +
										" (select discount from FORMTABLE_MAIN_141_DT1 where mainid = TB.id and cost_element = 2) as packagediscount, " +
										" (select a.NAME from MD_SD_SALES_AREA a LEFT JOIN MD_SD_FCST_CUST b on b.SALES_AREA_FK = a.ID where b.SALESMAN_FK = Createpersonnel) as salesarea, " +
										...
										" '' as failurereason, (select RECEIVEDATE from WORKFLOW_CURRENTOPERATOR where nodeid = '4881' and REQUESTID = tb.requestid and ISREJECT is null) as actuarialstarttime, " +
										" (select RECEIVEDATE from WORKFLOW_CURRENTOPERATOR where nodeid = '4882' and REQUESTID = tb.requestid and ISREJECT is null) as actuarialendtime, " +										
										" (select cost from FORMTABLE_MAIN_141_DT2 where mainid = TB.id and note = '加一根键合丝') as addbondingcost, " +
										" (select deal_price from FORMTABLE_MAIN_141_DT2 where mainid = TB.id and note = '加一根键合丝') as addbondingprice, " +
										" (select cost from FORMTABLE_MAIN_141_DT2 where mainid = TB.id and note = '加一个芯片') as addchipcost, " +
										" (select deal_price from FORMTABLE_MAIN_141_DT2 where mainid = TB.id and note = '加一个芯片') as addchipprice, " +
										" (select cost from FORMTABLE_MAIN_141_DT2 where mainid = TB.id and note = '金价波动100$') as goldvolatilecost, " +
										" (select deal_price from FORMTABLE_MAIN_141_DT2 where mainid = TB.id and note = '金价波动100$') as goldvolatileprice, " +
										" (select cost from FORMTABLE_MAIN_141_DT2 where mainid = TB.id and note = '铜价波动100$') as coppervolatilecost, " +
										" (select deal_price from FORMTABLE_MAIN_141_DT2 where mainid = TB.id and note = '铜价波动100$') as coppervolatileprice, " +										
										...
										" mat_pack, Custexpectprice, strikeprice, marketprice from FORMTABLE_MAIN_141 tb)";		            
