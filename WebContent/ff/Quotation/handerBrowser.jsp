...
String hander = Util.null2String(request.getParameter("hander"));
String sqlwhere = " 1=1 ";
	int ishead = 0;
...
if(!hander.equals("")){
			sqlwhere += "  and hander like '%" + hander + "%'";
	}
%>
...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="handerBrowser.jsp" method=post>
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'  attributes="{'groupSHBtnDisplay':'none'}" >
		<wea:item>机械手</wea:item>
		<wea:item><input name="hander" value='<%=hander%>' class="InputStyle"></wea:item>
	</wea:group>
</wea:layout>
</div>
...
String tableString = "";
int perpage=10;
String backfields = " distinct hander ";
String fromSql  = " (select b.name as hander, QTTN_PKG_FK from MD_SD_QTTN_PKG_HANDLER a right join MD_HANDLER b  on A.HANDLER_FK =b.id) ";
	tableString =   " <table instanceid=\"db_list3\" tabletype=\"none\" pagesize=\""+perpage+"\"  >"+ //指定分页条数和初始化id以及是否有复选框
			...
			" sqlprimarykey=\"hander\" sqlsortway=\"asc\" sqlisdistinct=\"false\" />"+
                "       <head>"+
                "           <col width=\"0%\" hide=\"true\"  text=\""+"hander"+"\" column=\"hander\"    />"+
	        	"           <col width=\"100%\"  text=\"机械手\" column=\"hander\" orderkey=\"hander\"   />"+
                "       </head>"+
                " </table>";
%>
