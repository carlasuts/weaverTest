...
String sqlwhere = " 1=1 and NAME = '框架工艺' ";
	int ishead = 0;
...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="frametechnologyBrowser.jsp" method=post>
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'  attributes="{'groupSHBtnDisplay':'none'}" >
		<wea:item>框架工艺</wea:item>
		<wea:item><input name="frame" value='<%=frame%>' class="InputStyle"></wea:item>
	</wea:group>
</wea:layout>
</div>
...
String tableString = "";
int perpage=10;
String backfields = " distinct frame, QTTN_PKG_FK, NAME ";
String fromSql  = " (select FULL_NAME as frame , QTTN_PKG_FK, b.NAME from MD_SD_COST_ASSY_OTHER_OPTION a left join  MD_SD_COST_ASSY_OTHER b on A.COST_ASSY_OTHER_FK =b.id) ";
	tableString =   " <table instanceid=\"db_list3\" tabletype=\"none\" pagesize=\""+perpage+"\"  >"+ //指定分页条数和初始化id以及是否有复选框
			...
			" sqlprimarykey=\"frame\" sqlsortway=\"asc\" sqlisdistinct=\"false\" />"+
                "       <head>"+
                "           <col width=\"0%\" hide=\"true\"  text=\""+"frame"+"\" column=\"frame\"    />"+
	        	"           <col width=\"100%\"  text=\"框架工艺\" column=\"frame\" orderkey=\"frame\"   />"+
                "       </head>"+
                " </table>";
%>
