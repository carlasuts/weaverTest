...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="formBrowser.jsp" method=post>
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'  attributes="{'groupSHBtnDisplay':'none'}" >
		<wea:item>包装形式</wea:item>
		<wea:item><input name="form" value='<%=form%>' class="InputStyle"></wea:item>
	</wea:group>
</wea:layout>
</div>
...
String tableString = "";
int perpage=10;
String backfields = " distinct form ";
String fromSql  = " (select b.name as form, QTTN_PKG_FK from MD_SD_COST_PACKING a left join MD_SD_PACKING b on A.PACKING_FK =b.id left join MD_SD_PACKING_GRADE c on A.PACKING_GRADE_FK =c.id) ";
	tableString = " <table instanceid=\"db_list3\" tabletype=\"none\" pagesize=\""+perpage+"\"  >" + //指定分页条数和初始化id以及是否有复选框
				...
				" sqlprimarykey=\"form\" sqlsortway=\"asc\" sqlisdistinct=\"false\" />" +
                "       <head>" +
                "           <col width=\"0%\" hide=\"true\"  text=\""+"form"+"\" column=\"form\"    />" +
	        	"           <col width=\"100%\"  text=\"包装形式\" column=\"form\" orderkey=\"form\"   />" +
                "       </head>" +
                " </table>";
%>
