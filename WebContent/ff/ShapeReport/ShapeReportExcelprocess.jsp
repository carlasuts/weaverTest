...
<%
	...
	StringBuilder sb = new StringBuilder();
     ...
     while( true ) {
	    ...
	    if(c2.equals("圆片厂")){	factory="0000";}
	    else if(c2.equals("一期厂")){	factory="1000";}
	    else if(c2.equals("二期厂")){	factory="2000";}
	    else if(c2.equals("三期厂")){	factory="3000";}
	    else if(c2.equals("合肥厂")){	factory="4000";}
	    else if(c2.equals("苏通厂")){	factory="5000";}
		...
		//根据封装外形id确定唯一
		sql = "select * from MD_SD_QTTN_PKG where id = '" + c3+"'";
		rs1.executeSql(sql);

		if(!rs1.next()){
			sql = "insert into MD_SD_QTTN_PKG (FACTORY_FK,id,name) values ("+
			"'" + factory + "'," +
			"'" + c3 + "'," +
			"'" + c4 + "'" +
			")";
		...
		sql = "update MD_SD_QTTN_PKG set id = '"+ c3 +"' ,name = '"+ c4 +"' ,FACTORY_FK = '"+ factory +"' where id = '" + c3+"'";
	    rs.executeSql(sql);
